classdef ComparisonGroupBrainPF_BB < ComparisonGroupBrainPF
	...
	%
	% The list of ComparisonGroupBrainPF_BB properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the  % % % .
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the panel figure binodal bilayer group comparison on brain surface figure.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the panel figure binodal bilayer group comparison on brain surface figure.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the panel figure binodal bilayer group comparison on brain surface figure.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the panel figure binodal bilayer group comparison on brain surface figure.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the panel figure binodal bilayer group comparison on brain surface figure.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the panel figure binodal bilayer group comparison on brain surface figure.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the panel binodal bilayer group comparison on brain surface figure.
			%
			% BUILD = ComparisonGroupBrainPF_BB.GETBUILD() returns the build of 'ComparisonGroupBrainPF_BB'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel binodal bilayer group comparison on brain surface figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonGroupBrainPF_BB') returns the build of 'ComparisonGroupBrainPF_BB'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonGroupBrainPF_BB')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonGroupBrainPF_BB' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupBrainPF_BB:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupBrainPF_BB:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisongroupbrainpf_bb_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'VIEW'  'ST_AXIS'  'LISTENER_ST_AXIS'  'SURFFILE'  'SURF'  'H_BRAIN'  'BRAIN'  'ST_SURFACE'  'ST_AMBIENT'  'BA'  'H_SPHS'  'SPHS'  'SPH_DICT'  'H_SYMS'  'SYMS'  'SYM_DICT'  'H_IDS'  'IDS'  'ID_DICT'  'H_LABS'  'LABS'  'LAB_DICT'  'CP'  'SETUP'  'NODES' };
				tag = comparisongroupbrainpf_bb_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongroupbrainpf_bb_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6  8 };
			prop_category = comparisongroupbrainpf_bb_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongroupbrainpf_bb_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1  12 };
			prop_format = comparisongroupbrainpf_bb_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 48 % ComparisonGroupBrainPF_BB.NODES
					prop_settings = Format.getFormatSettings(12);
				case 4 % ComparisonGroupBrainPF_BB.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 48 % ComparisonGroupBrainPF_BB.NODES
					prop_default = Format.getFormatDefault(12, ComparisonGroupBrainPF_BB.getPropSettings(prop));
				case 1 % ComparisonGroupBrainPF_BB.ELCLASS
					prop_default = 'ComparisonGroupBrainPF_BB';
				case 2 % ComparisonGroupBrainPF_BB.NAME
					prop_default = 'ComparisonGroupBrainPF_BB';
				case 3 % ComparisonGroupBrainPF_BB.DESCRIPTION
				...
				case 4 % ComparisonGroupBrainPF_BB.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonGroupBrainPF_BB.getPropSettings(prop));
				case 5 % ComparisonGroupBrainPF_BB.ID
					prop_default = 'ComparisonGroupBrainPF_BB ID';
				case 6 % ComparisonGroupBrainPF_BB.LABEL
					prop_default = 'ComparisonGroupBrainPF_BB label';
				case 7 % ComparisonGroupBrainPF_BB.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 48 % ComparisonGroupBrainPF_BB.NODES
					check = Format.checkFormat(12, value, ComparisonGroupBrainPF_BB.getPropSettings(prop));
				case 4 % ComparisonGroupBrainPF_BB.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonGroupBrainPF_BB.getPropSettings(prop));
				otherwise
					if prop <= 47
