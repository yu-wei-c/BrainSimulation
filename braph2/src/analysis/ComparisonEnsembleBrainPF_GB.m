classdef ComparisonEnsembleBrainPF_GB < ComparisonEnsembleBrainPF
	...
	%
	% The list of ComparisonEnsembleBrainPF_GB properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the % % % .
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the panel figure global bilayer ensemble-based comparison figure on brain surface.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the panel figure global bilayer ensemble-based comparison figure on brain surface.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the panel figure global bilayer ensemble-based comparison figure on brain surface.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the panel figure global bilayer ensemble-based comparison figure on brain surface.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the panel figure global bilayer ensemble-based comparison figure on brain surface.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the panel figure global bilayer ensemble-based comparison figure on brain surface.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the panel global bilayer ensemble-based comparison figure on brain surface figure.
			%
			% BUILD = ComparisonEnsembleBrainPF_GB.GETBUILD() returns the build of 'ComparisonEnsembleBrainPF_GB'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel global bilayer ensemble-based comparison figure on brain surface figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonEnsembleBrainPF_GB') returns the build of 'ComparisonEnsembleBrainPF_GB'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonEnsembleBrainPF_GB')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonEnsembleBrainPF_GB' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonEnsembleBrainPF_GB:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonEnsembleBrainPF_GB:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisonensemblebrainpf_gb_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'VIEW'  'ST_AXIS'  'LISTENER_ST_AXIS'  'SURFFILE'  'SURF'  'H_BRAIN'  'BRAIN'  'ST_SURFACE'  'ST_AMBIENT'  'BA'  'H_SPHS'  'SPHS'  'SPH_DICT'  'H_SYMS'  'SYMS'  'SYM_DICT'  'H_IDS'  'IDS'  'ID_DICT'  'H_LABS'  'LABS'  'LAB_DICT'  'CP'  'SETUP'  'NODES' };
				tag = comparisonensemblebrainpf_gb_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblebrainpf_gb_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6  8 };
			prop_category = comparisonensemblebrainpf_gb_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblebrainpf_gb_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1  12 };
			prop_format = comparisonensemblebrainpf_gb_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 48 % ComparisonEnsembleBrainPF_GB.NODES
					prop_settings = Format.getFormatSettings(12);
				case 4 % ComparisonEnsembleBrainPF_GB.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 48 % ComparisonEnsembleBrainPF_GB.NODES
					prop_default = Format.getFormatDefault(12, ComparisonEnsembleBrainPF_GB.getPropSettings(prop));
				case 1 % ComparisonEnsembleBrainPF_GB.ELCLASS
					prop_default = 'ComparisonEnsembleBrainPF_GB';
				case 2 % ComparisonEnsembleBrainPF_GB.NAME
					prop_default = 'ComparisonEnsembleBrainPF_GB';
				case 3 % ComparisonEnsembleBrainPF_GB.DESCRIPTION
				...
				case 4 % ComparisonEnsembleBrainPF_GB.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonEnsembleBrainPF_GB.getPropSettings(prop));
				case 5 % ComparisonEnsembleBrainPF_GB.ID
					prop_default = 'ComparisonEnsembleBrainPF_GB ID';
				case 6 % ComparisonEnsembleBrainPF_GB.LABEL
					prop_default = 'ComparisonEnsembleBrainPF_GB label';
				case 7 % ComparisonEnsembleBrainPF_GB.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 48 % ComparisonEnsembleBrainPF_GB.NODES
					check = Format.checkFormat(12, value, ComparisonEnsembleBrainPF_GB.getPropSettings(prop));
				case 4 % ComparisonEnsembleBrainPF_GB.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonEnsembleBrainPF_GB.getPropSettings(prop));
				otherwise
					if prop <= 47
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 47 % ComparisonEnsembleBrainPF_GB.SETUP
					warning([BRAPH2.STR ':ComparisonEnsembleBrainPF_GB'], [BRAPH2.STR ':ComparisonEnsembleBrainPF_GB \nThis functionality is not implemented yet.\nYou can contact the BRAPH2 developers and ask for it, \nor, even better, implement it yourself and share it with the community!'])
					value = [];
