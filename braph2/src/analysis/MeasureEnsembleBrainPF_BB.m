classdef MeasureEnsembleBrainPF_BB < MeasureEnsembleBrainPF
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = MeasureEnsembleBrainPF_BB.GETBUILD() returns the build of 'MeasureEnsembleBrainPF_BB'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for binodal bilayer measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureEnsembleBrainPF_BB') returns the build of 'MeasureEnsembleBrainPF_BB'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureEnsembleBrainPF_BB')
