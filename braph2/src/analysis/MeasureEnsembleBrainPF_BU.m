classdef MeasureEnsembleBrainPF_BU < MeasureEnsembleBrainPF
	%MeasureEnsembleBrainPF_BU is a plot of a binodal unilayer measure group brain.
	...
	%
	% The list of MeasureEnsembleBrainPF_BU properties is:
	...
	%
	% MeasureEnsembleBrainPF_BU methods (constructor):
	%  MeasureEnsembleBrainPF_BU - constructor
	%
	% MeasureEnsembleBrainPF_BU methods:
	...
	%
	% MeasureEnsembleBrainPF_BU methods (display):
	...
	%
	% MeasureEnsembleBrainPF_BU methods (miscellanea):
	...
	%
	% MeasureEnsembleBrainPF_BU methods (save/load, Static):
	...
	%
	% MeasureEnsembleBrainPF_BU methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% MeasureEnsembleBrainPF_BU methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% MeasureEnsembleBrainPF_BU methods (category, Static):
	...
	%
	% MeasureEnsembleBrainPF_BU methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?MeasureEnsembleBrainPF_BU; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">MeasureEnsembleBrainPF_BU constants</a>.
	%
	%
	...
	methods % constructor
		function pf = MeasureEnsembleBrainPF_BU(varargin)
			%MeasureEnsembleBrainPF_BU() creates a panel figure for binodal unilayer measure group brain.
			%
			% MeasureEnsembleBrainPF_BU(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% MeasureEnsembleBrainPF_BU(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = MeasureEnsembleBrainPF_BU.GETBUILD() returns the build of 'MeasureEnsembleBrainPF_BU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for binodal unilayer measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureEnsembleBrainPF_BU') returns the build of 'MeasureEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureEnsembleBrainPF_BU')
		...
		function pf_class = getClass()
			%GETCLASS returns the class of the panel figure for binodal unilayer measure group brain.
			%
			% CLASS = MeasureEnsembleBrainPF_BU.GETCLASS() returns the class 'MeasureEnsembleBrainPF_BU'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PF.GETCLASS() returns the class of the panel figure for binodal unilayer measure group brain PF.
			%  CLASS = Element.GETCLASS(PF) returns the class of 'PF'.
			%  CLASS = Element.GETCLASS('MeasureEnsembleBrainPF_BU') returns 'MeasureEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETCLASS(PF) and Element.GETCLASS('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			
			pf_class = 'MeasureEnsembleBrainPF_BU';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure for binodal unilayer measure group brain.
			%
			% LIST = MeasureEnsembleBrainPF_BU.GETSUBCLASSES() returns all subclasses of 'MeasureEnsembleBrainPF_BU'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel figure for binodal unilayer measure group brain PF.
			%  LIST = Element.GETSUBCLASSES(PF) returns all subclasses of 'PF'.
			%  LIST = Element.GETSUBCLASSES('MeasureEnsembleBrainPF_BU') returns all subclasses of 'MeasureEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETSUBCLASSES(PF) and Element.GETSUBCLASSES('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'MeasureEnsembleBrainPF_BU' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure for binodal unilayer measure group brain.
			%
			% PROPS = MeasureEnsembleBrainPF_BU.GETPROPS() returns the property list of panel figure for binodal unilayer measure group brain
			%  as a row vector.
			%
			% PROPS = MeasureEnsembleBrainPF_BU.GETPROPS(CATEGORY) returns the property list 
			...
			%  PROPS = Element.GETPROPS('MeasureEnsembleBrainPF_BU'[, CATEGORY]) returns the property list of 'MeasureEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETPROPS(PF) and Element.GETPROPS('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = MeasureEnsembleBrainPF_BU.GETPROPNUMBER() returns the property number of panel figure for binodal unilayer measure group brain.
			%
			% N = MeasureEnsembleBrainPF_BU.GETPROPNUMBER(CATEGORY) returns the property number of panel figure for binodal unilayer measure group brain
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			...
			%  N = Element.GETPROPNUMBER('MeasureEnsembleBrainPF_BU') returns the property number of 'MeasureEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETPROPNUMBER(PF) and Element.GETPROPNUMBER('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%
			% CHECK = MeasureEnsembleBrainPF_BU.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSPROP(PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(PF, PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(MeasureEnsembleBrainPF_BU, PROP) checks whether PROP exists for MeasureEnsembleBrainPF_BU.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			%  Element.EXISTSPROP(MeasureEnsembleBrainPF_BU, PROP) throws error if PROP does NOT exist for MeasureEnsembleBrainPF_BU.
			%   Error id: [BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			%
			% Note that the Element.EXISTSPROP(PF) and Element.EXISTSPROP('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput'], ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput' '\n' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for MeasureEnsembleBrainPF_BU.'] ...
					)
			end
		end
		function check_out = existsTag(tag)
			...
			%
			% CHECK = MeasureEnsembleBrainPF_BU.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSTAG(TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(PF, TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(MeasureEnsembleBrainPF_BU, TAG) checks whether TAG exists for MeasureEnsembleBrainPF_BU.
			%
			...
			%   Error id: [BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			%  Element.EXISTSTAG(MeasureEnsembleBrainPF_BU, TAG) throws error if TAG does NOT exist for MeasureEnsembleBrainPF_BU.
			%   Error id: [BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			%
			% Note that the Element.EXISTSTAG(PF) and Element.EXISTSTAG('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput'], ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput' '\n' ...
					'The value ' tag ' is not a valid tag for MeasureEnsembleBrainPF_BU.'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(MeasureEnsembleBrainPF_BU, POINTER) returns property number of POINTER of MeasureEnsembleBrainPF_BU.
			%  PROPERTY = PF.GETPROPPROP(MeasureEnsembleBrainPF_BU, POINTER) returns property number of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPPROP(PF) and Element.GETPROPPROP('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(MeasureEnsembleBrainPF_BU, POINTER) returns tag of POINTER of MeasureEnsembleBrainPF_BU.
			%  TAG = PF.GETPROPTAG(MeasureEnsembleBrainPF_BU, POINTER) returns tag of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPTAG(PF) and Element.GETPROPTAG('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(MeasureEnsembleBrainPF_BU, POINTER) returns category of POINTER of MeasureEnsembleBrainPF_BU.
			%  CATEGORY = PF.GETPROPCATEGORY(MeasureEnsembleBrainPF_BU, POINTER) returns category of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPCATEGORY(PF) and Element.GETPROPCATEGORY('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			%  getPropFormat, getPropDescription, getPropDefault, checkProp.
			
			prop = MeasureEnsembleBrainPF_BU.getPropProp(pointer);
			
			%CET: Computational Efficiency Trick
			measureensemblebrainpf_bu_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6 };
			prop_category = measureensemblebrainpf_bu_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(MeasureEnsembleBrainPF_BU, POINTER) returns format of POINTER of MeasureEnsembleBrainPF_BU.
			%  FORMAT = PF.GETPROPFORMAT(MeasureEnsembleBrainPF_BU, POINTER) returns format of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPFORMAT(PF) and Element.GETPROPFORMAT('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			measureensemblebrainpf_bu_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1 };
			prop_format = measureensemblebrainpf_bu_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(MeasureEnsembleBrainPF_BU, POINTER) returns description of POINTER of MeasureEnsembleBrainPF_BU.
			%  DESCRIPTION = PF.GETPROPDESCRIPTION(MeasureEnsembleBrainPF_BU, POINTER) returns description of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPDESCRIPTION(PF) and Element.GETPROPDESCRIPTION('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(MeasureEnsembleBrainPF_BU, POINTER) returns settings of POINTER of MeasureEnsembleBrainPF_BU.
			%  SETTINGS = PF.GETPROPSETTINGS(MeasureEnsembleBrainPF_BU, POINTER) returns settings of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPSETTINGS(PF) and Element.GETPROPSETTINGS('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % MeasureEnsembleBrainPF_BU.TEMPLATE
					prop_settings = 'MeasureEnsembleBrainPF_BU';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = MeasureEnsembleBrainPF_BU.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = MeasureEnsembleBrainPF_BU.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULT(MeasureEnsembleBrainPF_BU, POINTER) returns the default value of POINTER of MeasureEnsembleBrainPF_BU.
			%  DEFAULT = PF.GETPROPDEFAULT(MeasureEnsembleBrainPF_BU, POINTER) returns the default value of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPDEFAULT(PF) and Element.GETPROPDEFAULT('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % MeasureEnsembleBrainPF_BU.ELCLASS
					prop_default = 'MeasureEnsembleBrainPF_BU';
				case 2 % MeasureEnsembleBrainPF_BU.NAME
					prop_default = 'Panel Figure for Measure Group Brain';
				case 3 % MeasureEnsembleBrainPF_BU.DESCRIPTION
					prop_default = 'A Panel Figure for a Brain Measure of Analyze Group (MeasureEnsembleBrainPF_BU) manages the plot of the binodal unilayer measure ploted over the brain. MeasureEnsembleBrainPF_BU  utilizes the surface created from BrainAtlasPF to integrate the binodal unilayer meaure into the brain regions.';
				case 4 % MeasureEnsembleBrainPF_BU.TEMPLATE
					prop_default = Format.getFormatDefault(8, MeasureEnsembleBrainPF_BU.getPropSettings(prop));
				case 5 % MeasureEnsembleBrainPF_BU.ID
					prop_default = 'MeasureEnsembleBrainPF_BU ID';
				case 6 % MeasureEnsembleBrainPF_BU.LABEL
					prop_default = 'MeasureEnsembleBrainPF_BU label';
				case 7 % MeasureEnsembleBrainPF_BU.NOTES
					prop_default = 'MeasureEnsembleBrainPF_BU notes';
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = MeasureEnsembleBrainPF_BU.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = MeasureEnsembleBrainPF_BU.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(MeasureEnsembleBrainPF_BU, POINTER) returns the conditioned default value of POINTER of MeasureEnsembleBrainPF_BU.
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(MeasureEnsembleBrainPF_BU, POINTER) returns the conditioned default value of POINTER of MeasureEnsembleBrainPF_BU.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(PF) and Element.GETPROPDEFAULTCONDITIONED('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			prop_default = MeasureEnsembleBrainPF_BU.conditioning(prop, MeasureEnsembleBrainPF_BU.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(MeasureEnsembleBrainPF_BU, PROP, VALUE) checks VALUE format for PROP of MeasureEnsembleBrainPF_BU.
			%  CHECK = PF.CHECKPROP(MeasureEnsembleBrainPF_BU, PROP, VALUE) checks VALUE format for PROP of MeasureEnsembleBrainPF_BU.
			% 
			...
			%   Error id: BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput
			%  Element.CHECKPROP(MeasureEnsembleBrainPF_BU, PROP, VALUE) throws error if VALUE has not a valid format for PROP of MeasureEnsembleBrainPF_BU.
			%   Error id: BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput
			%  PF.CHECKPROP(MeasureEnsembleBrainPF_BU, PROP, VALUE) throws error if VALUE has not a valid format for PROP of MeasureEnsembleBrainPF_BU.
			%   Error id: BRAPH2:MeasureEnsembleBrainPF_BU:WrongInput]
			% 
			% Note that the Element.CHECKPROP(PF) and Element.CHECKPROP('MeasureEnsembleBrainPF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 4 % MeasureEnsembleBrainPF_BU.TEMPLATE
					check = Format.checkFormat(8, value, MeasureEnsembleBrainPF_BU.getPropSettings(prop));
				otherwise
					if prop <= 47
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput'], ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BU:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' MeasureEnsembleBrainPF_BU.getPropTag(prop) ' (' MeasureEnsembleBrainPF_BU.getFormatTag(MeasureEnsembleBrainPF_BU.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
