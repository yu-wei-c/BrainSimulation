classdef MeasureEnsembleBrainPF_BS < MeasureEnsembleBrainPF
	...
	%
	% The list of MeasureEnsembleBrainPF_BS properties is:
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = MeasureEnsembleBrainPF_BS.GETBUILD() returns the build of 'MeasureEnsembleBrainPF_BS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for binodal superglobal measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureEnsembleBrainPF_BS') returns the build of 'MeasureEnsembleBrainPF_BS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureEnsembleBrainPF_BS')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'MeasureEnsembleBrainPF_BS' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BS:' 'WrongInput'], ...
					['BRAPH2' ':MeasureEnsembleBrainPF_BS:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				measureensemblebrainpf_bs_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'VIEW'  'ST_AXIS'  'LISTENER_ST_AXIS'  'SURFFILE'  'SURF'  'H_BRAIN'  'BRAIN'  'ST_SURFACE'  'ST_AMBIENT'  'BA'  'H_SPHS'  'SPHS'  'SPH_DICT'  'H_SYMS'  'SYMS'  'SYM_DICT'  'H_IDS'  'IDS'  'ID_DICT'  'H_LABS'  'LABS'  'LAB_DICT'  'ME'  'SETUP' };
				tag = measureensemblebrainpf_bs_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			measureensemblebrainpf_bs_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6 };
			prop_category = measureensemblebrainpf_bs_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			measureensemblebrainpf_bs_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1 };
			prop_format = measureensemblebrainpf_bs_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % MeasureEnsembleBrainPF_BS.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % MeasureEnsembleBrainPF_BS.ELCLASS
					prop_default = 'MeasureEnsembleBrainPF_BS';
				case 2 % MeasureEnsembleBrainPF_BS.NAME
					prop_default = 'Panel Figure for Measure Group Brain';
				case 3 % MeasureEnsembleBrainPF_BS.DESCRIPTION
				...
				case 4 % MeasureEnsembleBrainPF_BS.TEMPLATE
					prop_default = Format.getFormatDefault(8, MeasureEnsembleBrainPF_BS.getPropSettings(prop));
				case 5 % MeasureEnsembleBrainPF_BS.ID
					prop_default = 'MeasureEnsembleBrainPF_BS ID';
				case 6 % MeasureEnsembleBrainPF_BS.LABEL
					prop_default = 'MeasureEnsembleBrainPF_BS label';
				case 7 % MeasureEnsembleBrainPF_BS.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 4 % MeasureEnsembleBrainPF_BS.TEMPLATE
					check = Format.checkFormat(8, value, MeasureEnsembleBrainPF_BS.getPropSettings(prop));
				otherwise
					if prop <= 47
