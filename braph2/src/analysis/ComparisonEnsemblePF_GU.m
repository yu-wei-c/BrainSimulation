classdef ComparisonEnsemblePF_GU < ComparisonEnsemblePF
	...
	%
	% The list of ComparisonEnsemblePF_GU properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ComparisonEnsemblePF_GU.
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = ComparisonEnsemblePF_GU.GETBUILD() returns the build of 'ComparisonEnsemblePF_GU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel global unilayer group comparison figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonEnsemblePF_GU') returns the build of 'ComparisonEnsemblePF_GU'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonEnsemblePF_GU')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonEnsemblePF_GU' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonEnsemblePF_GU:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonEnsemblePF_GU:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisonensemblepf_gu_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'ST_AXIS'  'LISTENER_ST_AXIS'  'CP'  'SETUP'  'H_AREA'  'ST_AREA'  'LISTENER_ST_AREA'  'H_LINE_DIFF'  'ST_LINE_DIFF'  'LISTENER_ST_LINE_DIFF'  'H_LINE_CIL'  'ST_LINE_CIL'  'LISTENER_ST_LINE_CIL'  'H_LINE_CIU'  'ST_LINE_CIU'  'LISTENER_ST_LINE_CIU'  'H_TITLE'  'ST_TITLE'  'H_XLABEL'  'ST_XLABEL'  'H_YLABEL'  'ST_YLABEL'  'LAYER' };
				tag = comparisonensemblepf_gu_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblepf_gu_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8  8 };
			prop_category = comparisonensemblepf_gu_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblepf_gu_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8  11 };
			prop_format = comparisonensemblepf_gu_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonEnsemblePF_GU.LAYER
					prop_settings = Format.getFormatSettings(11);
				case 4 % ComparisonEnsemblePF_GU.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonEnsemblePF_GU.LAYER
					prop_default = 1;
				case 1 % ComparisonEnsemblePF_GU.ELCLASS
					prop_default = 'ComparisonEnsemblePF_GU';
				case 2 % ComparisonEnsemblePF_GU.NAME
					prop_default = 'ComparisonEnsemblePF_GU';
				case 3 % ComparisonEnsemblePF_GU.DESCRIPTION
				...
				case 4 % ComparisonEnsemblePF_GU.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonEnsemblePF_GU.getPropSettings(prop));
				case 5 % ComparisonEnsemblePF_GU.ID
					prop_default = 'ComparisonEnsemblePF_GU ID';
				case 6 % ComparisonEnsemblePF_GU.LABEL
					prop_default = 'ComparisonEnsemblePF_GU label';
				case 7 % ComparisonEnsemblePF_GU.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 46 % ComparisonEnsemblePF_GU.LAYER
					check = Format.checkFormat(11, value, ComparisonEnsemblePF_GU.getPropSettings(prop));
				case 4 % ComparisonEnsemblePF_GU.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonEnsemblePF_GU.getPropSettings(prop));
				otherwise
					if prop <= 45
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 27 % ComparisonEnsemblePF_GU.SETUP
					cp = pf.get('CP');
	...
	methods % GUI
		function pr = getPanelProp(pf, prop, varargin)
			...
			switch prop
				case 46 % ComparisonEnsemblePF_GU.LAYER
					pr = ComparisonEnsemblePF_xUPP_Layer('EL', pf, 'PROP', 46);
