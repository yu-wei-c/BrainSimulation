classdef MeasureGroupBrainPF_GS < MeasureGroupBrainPF
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = MeasureGroupBrainPF_GS.GETBUILD() returns the build of 'MeasureGroupBrainPF_GS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for global superglobal measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureGroupBrainPF_GS') returns the build of 'MeasureGroupBrainPF_GS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureGroupBrainPF_GS')
