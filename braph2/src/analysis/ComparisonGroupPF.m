classdef ComparisonGroupPF < PanelFig
	...
	%
	% The list of ComparisonGroupPF properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the % % % .
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the panel group comparison figure.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the panel group comparison figure.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the panel group comparison figure.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the panel group comparison figure.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the panel group comparison figure.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the panel group comparison figure.
	...
	%  <strong>27</strong> <strong>SETUP</strong> 	SETUP (query, empty) calculates the group comparison value and stores it to be implemented in the subelements.
	...
	methods (Static) % inspection
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisongrouppf_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'ST_AXIS'  'LISTENER_ST_AXIS'  'CP'  'SETUP'  'H_AREA'  'ST_AREA'  'LISTENER_ST_AREA'  'H_LINE_DIFF'  'ST_LINE_DIFF'  'LISTENER_ST_LINE_DIFF'  'H_LINE_CIL'  'ST_LINE_CIL'  'LISTENER_ST_LINE_CIL'  'H_LINE_CIU'  'ST_LINE_CIU'  'LISTENER_ST_LINE_CIU'  'H_TITLE'  'ST_TITLE'  'H_XLABEL'  'ST_XLABEL'  'H_YLABEL'  'ST_YLABEL' };
				tag = comparisongrouppf_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8 };
			prop_category = comparisongrouppf_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8 };
			prop_format = comparisongrouppf_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 23 % ComparisonGroupPF.H_AXES
					prop_settings = Format.getFormatSettings(18);
				case 24 % ComparisonGroupPF.ST_AXIS
					prop_settings = 'SettingsAxis';
				case 25 % ComparisonGroupPF.LISTENER_ST_AXIS
					prop_settings = Format.getFormatSettings(18);
				case 26 % ComparisonGroupPF.CP
					prop_settings = 'ComparisonGroup';
				case 27 % ComparisonGroupPF.SETUP
					prop_settings = Format.getFormatSettings(1);
				case 28 % ComparisonGroupPF.H_AREA
					prop_settings = Format.getFormatSettings(18);
				case 29 % ComparisonGroupPF.ST_AREA
					prop_settings = 'SettingsArea';
				case 30 % ComparisonGroupPF.LISTENER_ST_AREA
					prop_settings = Format.getFormatSettings(18);
				case 31 % ComparisonGroupPF.H_LINE_DIFF
					prop_settings = Format.getFormatSettings(18);
				case 32 % ComparisonGroupPF.ST_LINE_DIFF
					prop_settings = 'SettingsLine';
				case 33 % ComparisonGroupPF.LISTENER_ST_LINE_DIFF
					prop_settings = Format.getFormatSettings(18);
				case 34 % ComparisonGroupPF.H_LINE_CIL
					prop_settings = Format.getFormatSettings(18);
				case 35 % ComparisonGroupPF.ST_LINE_CIL
					prop_settings = 'SettingsLine';
				case 36 % ComparisonGroupPF.LISTENER_ST_LINE_CIL
					prop_settings = Format.getFormatSettings(18);
				case 37 % ComparisonGroupPF.H_LINE_CIU
					prop_settings = Format.getFormatSettings(18);
				case 38 % ComparisonGroupPF.ST_LINE_CIU
					prop_settings = 'SettingsLine';
				case 39 % ComparisonGroupPF.LISTENER_ST_LINE_CIU
					prop_settings = Format.getFormatSettings(18);
				case 40 % ComparisonGroupPF.H_TITLE
					prop_settings = Format.getFormatSettings(18);
				case 41 % ComparisonGroupPF.ST_TITLE
					prop_settings = 'SettingsText';
				case 42 % ComparisonGroupPF.H_XLABEL
					prop_settings = Format.getFormatSettings(18);
				case 43 % ComparisonGroupPF.ST_XLABEL
					prop_settings = 'SettingsText';
				case 44 % ComparisonGroupPF.H_YLABEL
					prop_settings = Format.getFormatSettings(18);
				case 45 % ComparisonGroupPF.ST_YLABEL
					prop_settings = 'SettingsText';
				case 4 % ComparisonGroupPF.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 23 % ComparisonGroupPF.H_AXES
					prop_default = Format.getFormatDefault(18, ComparisonGroupPF.getPropSettings(prop));
				case 24 % ComparisonGroupPF.ST_AXIS
					prop_default = SettingsAxis('AXIS', true, 'GRID', false, 'EQUAL', false);
				case 25 % ComparisonGroupPF.LISTENER_ST_AXIS
					prop_default = Format.getFormatDefault(18, ComparisonGroupPF.getPropSettings(prop));
				case 26 % ComparisonGroupPF.CP
					prop_default = Format.getFormatDefault(8, ComparisonGroupPF.getPropSettings(prop));
				case 27 % ComparisonGroupPF.SETUP
					prop_default = Format.getFormatDefault(1, ComparisonGroupPF.getPropSettings(prop));
				case 28 % ComparisonGroupPF.H_AREA
					prop_default = Format.getFormatDefault(18, ComparisonGroupPF.getPropSettings(prop));
				case 29 % ComparisonGroupPF.ST_AREA
					prop_default = Format.getFormatDefault(8, ComparisonGroupPF.getPropSettings(prop));
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 23 % ComparisonGroupPF.H_AXES
					check = Format.checkFormat(18, value, ComparisonGroupPF.getPropSettings(prop));
				case 24 % ComparisonGroupPF.ST_AXIS
					check = Format.checkFormat(8, value, ComparisonGroupPF.getPropSettings(prop));
				case 25 % ComparisonGroupPF.LISTENER_ST_AXIS
					check = Format.checkFormat(18, value, ComparisonGroupPF.getPropSettings(prop));
				case 26 % ComparisonGroupPF.CP
					check = Format.checkFormat(8, value, ComparisonGroupPF.getPropSettings(prop));
				case 27 % ComparisonGroupPF.SETUP
					check = Format.checkFormat(1, value, ComparisonGroupPF.getPropSettings(prop));
				case 28 % ComparisonGroupPF.H_AREA
					check = Format.checkFormat(18, value, ComparisonGroupPF.getPropSettings(prop));
				case 29 % ComparisonGroupPF.ST_AREA
					check = Format.checkFormat(8, value, ComparisonGroupPF.getPropSettings(prop));
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				...
				case 11 % ComparisonGroupPF.DRAW
				...
				case 18 % ComparisonGroupPF.DELETE
				...
				case 22 % ComparisonGroupPF.H_TOOLS
