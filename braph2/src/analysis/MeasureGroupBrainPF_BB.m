classdef MeasureGroupBrainPF_BB < MeasureGroupBrainPF
	...
	%
	% The list of MeasureGroupBrainPF_BB properties is:
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the panel figure for binodal bilayer measure group brain.
			%
			% BUILD = MeasureGroupBrainPF_BB.GETBUILD() returns the build of 'MeasureGroupBrainPF_BB'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for binodal bilayer measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureGroupBrainPF_BB') returns the build of 'MeasureGroupBrainPF_BB'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureGroupBrainPF_BB')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'MeasureGroupBrainPF_BB' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureGroupBrainPF_BB:' 'WrongInput'], ...
					['BRAPH2' ':MeasureGroupBrainPF_BB:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				measuregroupbrainpf_bb_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'VIEW'  'ST_AXIS'  'LISTENER_ST_AXIS'  'SURFFILE'  'SURF'  'H_BRAIN'  'BRAIN'  'ST_SURFACE'  'ST_AMBIENT'  'BA'  'H_SPHS'  'SPHS'  'SPH_DICT'  'H_SYMS'  'SYMS'  'SYM_DICT'  'H_IDS'  'IDS'  'ID_DICT'  'H_LABS'  'LABS'  'LAB_DICT'  'M'  'SETUP' };
				tag = measuregroupbrainpf_bb_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			measuregroupbrainpf_bb_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6 };
			prop_category = measuregroupbrainpf_bb_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			measuregroupbrainpf_bb_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1 };
			prop_format = measuregroupbrainpf_bb_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % MeasureGroupBrainPF_BB.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % MeasureGroupBrainPF_BB.ELCLASS
					prop_default = 'MeasureGroupBrainPF_BB';
				case 2 % MeasureGroupBrainPF_BB.NAME
					prop_default = 'Panel Figure for Measure Group Brain';
				case 3 % MeasureGroupBrainPF_BB.DESCRIPTION
				...
				case 4 % MeasureGroupBrainPF_BB.TEMPLATE
					prop_default = Format.getFormatDefault(8, MeasureGroupBrainPF_BB.getPropSettings(prop));
				case 5 % MeasureGroupBrainPF_BB.ID
					prop_default = 'MeasureGroupBrainPF_BB ID';
				case 6 % MeasureGroupBrainPF_BB.LABEL
					prop_default = 'MeasureGroupBrainPF_BB label';
				case 7 % MeasureGroupBrainPF_BB.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 4 % MeasureGroupBrainPF_BB.TEMPLATE
					check = Format.checkFormat(8, value, MeasureGroupBrainPF_BB.getPropSettings(prop));
				otherwise
					if prop <= 47
