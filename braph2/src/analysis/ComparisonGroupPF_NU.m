classdef ComparisonGroupPF_NU < ComparisonGroupPF
	...
	%
	% The list of ComparisonGroupPF_NU properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ComparisonGroupPF_NU.
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = ComparisonGroupPF_NU.GETBUILD() returns the build of 'ComparisonGroupPF_NU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel nodal unilayer group comparison figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonGroupPF_NU') returns the build of 'ComparisonGroupPF_NU'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonGroupPF_NU')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonGroupPF_NU' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF_NU:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF_NU:' 'WrongInput' '\n' ...
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_nu_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8  8  8 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_nu_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8  11  11 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonGroupPF_NU.NODE
					prop_settings = Format.getFormatSettings(11);
				case 47 % ComparisonGroupPF_NU.LAYER
					prop_settings = Format.getFormatSettings(11);
				case 4 % ComparisonGroupPF_NU.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 47 % ComparisonGroupPF_NU.LAYER
					prop_default = 1;
				case 1 % ComparisonGroupPF_NU.ELCLASS
					prop_default = 'ComparisonGroupPF_NU';
				case 2 % ComparisonGroupPF_NU.NAME
					prop_default = 'ComparisonGroupPF_NU';
				case 3 % ComparisonGroupPF_NU.DESCRIPTION
					prop_default = 'ComparisonGroupPF_NU manages the basic functionalities to plot of a nodal unilayer group comparison figure.';
				case 4 % ComparisonGroupPF_NU.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonGroupPF_NU.getPropSettings(prop));
				case 5 % ComparisonGroupPF_NU.ID
					prop_default = 'ComparisonGroupPF_NU ID';
				case 6 % ComparisonGroupPF_NU.LABEL
					prop_default = 'ComparisonGroupPF_NU label';
				case 7 % ComparisonGroupPF_NU.NOTES
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 27 % ComparisonGroupPF_NU.SETUP
					cp = pf.get('CP');
