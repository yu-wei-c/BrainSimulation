classdef ComparisonEnsembleBrainPF_BU < ComparisonEnsembleBrainPF
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the panel binodal unilayer ensemble-based comparison figure on brain surface figure.
			%
			% BUILD = ComparisonEnsembleBrainPF_BU.GETBUILD() returns the build of 'ComparisonEnsembleBrainPF_BU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel binodal unilayer ensemble-based comparison figure on brain surface figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonEnsembleBrainPF_BU') returns the build of 'ComparisonEnsembleBrainPF_BU'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonEnsembleBrainPF_BU')
