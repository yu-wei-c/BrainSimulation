classdef ComparisonEnsemblePF_BS < ComparisonEnsemblePF
	...
	%
	% The list of ComparisonEnsemblePF_BS properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ComparisonEnsemblePF_BS.
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = ComparisonEnsemblePF_BS.GETBUILD() returns the build of 'ComparisonEnsemblePF_BS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel binodal superglobal group comparison figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonEnsemblePF_BS') returns the build of 'ComparisonEnsemblePF_BS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonEnsemblePF_BS')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonEnsemblePF_BS' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonEnsemblePF_BS:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonEnsemblePF_BS:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisonensemblepf_bs_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'ST_AXIS'  'LISTENER_ST_AXIS'  'CP'  'SETUP'  'H_AREA'  'ST_AREA'  'LISTENER_ST_AREA'  'H_LINE_DIFF'  'ST_LINE_DIFF'  'LISTENER_ST_LINE_DIFF'  'H_LINE_CIL'  'ST_LINE_CIL'  'LISTENER_ST_LINE_CIL'  'H_LINE_CIU'  'ST_LINE_CIU'  'LISTENER_ST_LINE_CIU'  'H_TITLE'  'ST_TITLE'  'H_XLABEL'  'ST_XLABEL'  'H_YLABEL'  'ST_YLABEL'  'NODES' };
				tag = comparisonensemblepf_bs_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblepf_bs_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8  8 };
			prop_category = comparisonensemblepf_bs_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisonensemblepf_bs_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8  12 };
			prop_format = comparisonensemblepf_bs_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonEnsemblePF_BS.NODES
					prop_settings = Format.getFormatSettings(12);
				case 4 % ComparisonEnsemblePF_BS.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonEnsemblePF_BS.NODES
					prop_default = [1 1];
				case 1 % ComparisonEnsemblePF_BS.ELCLASS
					prop_default = 'ComparisonEnsemblePF_BS';
				case 2 % ComparisonEnsemblePF_BS.NAME
					prop_default = 'ComparisonEnsemblePF_BS';
				case 3 % ComparisonEnsemblePF_BS.DESCRIPTION
				...
				case 4 % ComparisonEnsemblePF_BS.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonEnsemblePF_BS.getPropSettings(prop));
				case 5 % ComparisonEnsemblePF_BS.ID
					prop_default = 'ComparisonEnsemblePF_BS ID';
				case 6 % ComparisonEnsemblePF_BS.LABEL
					prop_default = 'ComparisonEnsemblePF_BS label';
				case 7 % ComparisonEnsemblePF_BS.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 46 % ComparisonEnsemblePF_BS.NODES
					check = Format.checkFormat(12, value, ComparisonEnsemblePF_BS.getPropSettings(prop));
				case 4 % ComparisonEnsemblePF_BS.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonEnsemblePF_BS.getPropSettings(prop));
				otherwise
					if prop <= 45
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 27 % ComparisonEnsemblePF_BS.SETUP
					cp = pf.get('CP');
