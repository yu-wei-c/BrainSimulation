classdef MeasureGroupBrainPF_BS < MeasureGroupBrainPF
	%MeasureGroupBrainPF_BS is a plot of a binodal superglobal measure group brain.
	...
	%
	% The list of MeasureGroupBrainPF_BS properties is:
	...
	%
	% MeasureGroupBrainPF_BS methods (constructor):
	%  MeasureGroupBrainPF_BS - constructor
	%
	% MeasureGroupBrainPF_BS methods:
	...
	%
	% MeasureGroupBrainPF_BS methods (display):
	...
	%
	% MeasureGroupBrainPF_BS methods (miscellanea):
	...
	%
	% MeasureGroupBrainPF_BS methods (save/load, Static):
	...
	%
	% MeasureGroupBrainPF_BS methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% MeasureGroupBrainPF_BS methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% MeasureGroupBrainPF_BS methods (category, Static):
	...
	%
	% MeasureGroupBrainPF_BS methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?MeasureGroupBrainPF_BS; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">MeasureGroupBrainPF_BS constants</a>.
	%
	%
	...
	methods % constructor
		function pf = MeasureGroupBrainPF_BS(varargin)
			%MeasureGroupBrainPF_BS() creates a panel figure for binodal superglobal measure group brain.
			%
			% MeasureGroupBrainPF_BS(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% MeasureGroupBrainPF_BS(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the panel figure for binodal superglobal measure group brain.
			%
			% BUILD = MeasureGroupBrainPF_BS.GETBUILD() returns the build of 'MeasureGroupBrainPF_BS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for binodal superglobal measure group brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('MeasureGroupBrainPF_BS') returns the build of 'MeasureGroupBrainPF_BS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('MeasureGroupBrainPF_BS')
		...
		function pf_class = getClass()
			%GETCLASS returns the class of the panel figure for binodal superglobal measure group brain.
			%
			% CLASS = MeasureGroupBrainPF_BS.GETCLASS() returns the class 'MeasureGroupBrainPF_BS'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PF.GETCLASS() returns the class of the panel figure for binodal superglobal measure group brain PF.
			%  CLASS = Element.GETCLASS(PF) returns the class of 'PF'.
			%  CLASS = Element.GETCLASS('MeasureGroupBrainPF_BS') returns 'MeasureGroupBrainPF_BS'.
			%
			% Note that the Element.GETCLASS(PF) and Element.GETCLASS('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			
			pf_class = 'MeasureGroupBrainPF_BS';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure for binodal superglobal measure group brain.
			%
			% LIST = MeasureGroupBrainPF_BS.GETSUBCLASSES() returns all subclasses of 'MeasureGroupBrainPF_BS'.
			%
			% Alternative forms to call this method are:
			...
			%  LIST = Element.GETSUBCLASSES('MeasureGroupBrainPF_BS') returns all subclasses of 'MeasureGroupBrainPF_BS'.
			%
			% Note that the Element.GETSUBCLASSES(PF) and Element.GETSUBCLASSES('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'MeasureGroupBrainPF_BS' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure for binodal superglobal measure group brain.
			%
			% PROPS = MeasureGroupBrainPF_BS.GETPROPS() returns the property list of panel figure for binodal superglobal measure group brain
			%  as a row vector.
			%
			% PROPS = MeasureGroupBrainPF_BS.GETPROPS(CATEGORY) returns the property list 
			...
			%  PROPS = Element.GETPROPS('MeasureGroupBrainPF_BS'[, CATEGORY]) returns the property list of 'MeasureGroupBrainPF_BS'.
			%
			% Note that the Element.GETPROPS(PF) and Element.GETPROPS('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = MeasureGroupBrainPF_BS.GETPROPNUMBER() returns the property number of panel figure for binodal superglobal measure group brain.
			%
			% N = MeasureGroupBrainPF_BS.GETPROPNUMBER(CATEGORY) returns the property number of panel figure for binodal superglobal measure group brain
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			...
			%  N = Element.GETPROPNUMBER('MeasureGroupBrainPF_BS') returns the property number of 'MeasureGroupBrainPF_BS'.
			%
			% Note that the Element.GETPROPNUMBER(PF) and Element.GETPROPNUMBER('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%
			% CHECK = MeasureGroupBrainPF_BS.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSPROP(PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(PF, PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(MeasureGroupBrainPF_BS, PROP) checks whether PROP exists for MeasureGroupBrainPF_BS.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			%  Element.EXISTSPROP(MeasureGroupBrainPF_BS, PROP) throws error if PROP does NOT exist for MeasureGroupBrainPF_BS.
			%   Error id: [BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			%
			% Note that the Element.EXISTSPROP(PF) and Element.EXISTSPROP('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput'], ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput' '\n' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for MeasureGroupBrainPF_BS.'] ...
					)
			end
		end
		function check_out = existsTag(tag)
			...
			%
			% CHECK = MeasureGroupBrainPF_BS.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSTAG(TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(PF, TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(MeasureGroupBrainPF_BS, TAG) checks whether TAG exists for MeasureGroupBrainPF_BS.
			%
			...
			%   Error id: [BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			%  Element.EXISTSTAG(MeasureGroupBrainPF_BS, TAG) throws error if TAG does NOT exist for MeasureGroupBrainPF_BS.
			%   Error id: [BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			%
			% Note that the Element.EXISTSTAG(PF) and Element.EXISTSTAG('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput'], ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput' '\n' ...
					'The value ' tag ' is not a valid tag for MeasureGroupBrainPF_BS.'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(MeasureGroupBrainPF_BS, POINTER) returns property number of POINTER of MeasureGroupBrainPF_BS.
			%  PROPERTY = PF.GETPROPPROP(MeasureGroupBrainPF_BS, POINTER) returns property number of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPPROP(PF) and Element.GETPROPPROP('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(MeasureGroupBrainPF_BS, POINTER) returns tag of POINTER of MeasureGroupBrainPF_BS.
			%  TAG = PF.GETPROPTAG(MeasureGroupBrainPF_BS, POINTER) returns tag of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPTAG(PF) and Element.GETPROPTAG('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				measuregroupbrainpf_bs_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'VIEW'  'ST_AXIS'  'LISTENER_ST_AXIS'  'SURFFILE'  'SURF'  'H_BRAIN'  'BRAIN'  'ST_SURFACE'  'ST_AMBIENT'  'BA'  'H_SPHS'  'SPHS'  'SPH_DICT'  'H_SYMS'  'SYMS'  'SYM_DICT'  'H_IDS'  'IDS'  'ID_DICT'  'H_LABS'  'LABS'  'LAB_DICT'  'M'  'SETUP' };
				tag = measuregroupbrainpf_bs_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(MeasureGroupBrainPF_BS, POINTER) returns category of POINTER of MeasureGroupBrainPF_BS.
			%  CATEGORY = PF.GETPROPCATEGORY(MeasureGroupBrainPF_BS, POINTER) returns category of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPCATEGORY(PF) and Element.GETPROPCATEGORY('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			%  getPropFormat, getPropDescription, getPropDefault, checkProp.
			
			prop = MeasureGroupBrainPF_BS.getPropProp(pointer);
			
			%CET: Computational Efficiency Trick
			measuregroupbrainpf_bs_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  6 };
			prop_category = measuregroupbrainpf_bs_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(MeasureGroupBrainPF_BS, POINTER) returns format of POINTER of MeasureGroupBrainPF_BS.
			%  FORMAT = PF.GETPROPFORMAT(MeasureGroupBrainPF_BS, POINTER) returns format of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPFORMAT(PF) and Element.GETPROPFORMAT('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			measuregroupbrainpf_bs_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  8  1 };
			prop_format = measuregroupbrainpf_bs_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(MeasureGroupBrainPF_BS, POINTER) returns description of POINTER of MeasureGroupBrainPF_BS.
			%  DESCRIPTION = PF.GETPROPDESCRIPTION(MeasureGroupBrainPF_BS, POINTER) returns description of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPDESCRIPTION(PF) and Element.GETPROPDESCRIPTION('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(MeasureGroupBrainPF_BS, POINTER) returns settings of POINTER of MeasureGroupBrainPF_BS.
			%  SETTINGS = PF.GETPROPSETTINGS(MeasureGroupBrainPF_BS, POINTER) returns settings of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPSETTINGS(PF) and Element.GETPROPSETTINGS('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % MeasureGroupBrainPF_BS.TEMPLATE
					prop_settings = 'MeasureGroupBrainPF_BS';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = MeasureGroupBrainPF_BS.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = MeasureGroupBrainPF_BS.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULT(MeasureGroupBrainPF_BS, POINTER) returns the default value of POINTER of MeasureGroupBrainPF_BS.
			%  DEFAULT = PF.GETPROPDEFAULT(MeasureGroupBrainPF_BS, POINTER) returns the default value of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPDEFAULT(PF) and Element.GETPROPDEFAULT('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % MeasureGroupBrainPF_BS.ELCLASS
					prop_default = 'MeasureGroupBrainPF_BS';
				case 2 % MeasureGroupBrainPF_BS.NAME
					prop_default = 'Panel Figure for Measure Group Brain';
				case 3 % MeasureGroupBrainPF_BS.DESCRIPTION
					prop_default = 'A Panel Figure for a Brain Measure of Analyze Group (MeasureGroupBrainPF_BS) manages the plot of the binodal superglobal measure ploted over the brain. MeasureGroupBrainPF_BS  utilizes the surface created from BrainAtlasPF to integrate the binodal superglobal meaure into the brain regions.';
				case 4 % MeasureGroupBrainPF_BS.TEMPLATE
					prop_default = Format.getFormatDefault(8, MeasureGroupBrainPF_BS.getPropSettings(prop));
				case 5 % MeasureGroupBrainPF_BS.ID
					prop_default = 'MeasureGroupBrainPF_BS ID';
				case 6 % MeasureGroupBrainPF_BS.LABEL
					prop_default = 'MeasureGroupBrainPF_BS label';
				case 7 % MeasureGroupBrainPF_BS.NOTES
					prop_default = 'MeasureGroupBrainPF_BS notes';
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = MeasureGroupBrainPF_BS.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = MeasureGroupBrainPF_BS.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(MeasureGroupBrainPF_BS, POINTER) returns the conditioned default value of POINTER of MeasureGroupBrainPF_BS.
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(MeasureGroupBrainPF_BS, POINTER) returns the conditioned default value of POINTER of MeasureGroupBrainPF_BS.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(PF) and Element.GETPROPDEFAULTCONDITIONED('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			prop_default = MeasureGroupBrainPF_BS.conditioning(prop, MeasureGroupBrainPF_BS.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(MeasureGroupBrainPF_BS, PROP, VALUE) checks VALUE format for PROP of MeasureGroupBrainPF_BS.
			%  CHECK = PF.CHECKPROP(MeasureGroupBrainPF_BS, PROP, VALUE) checks VALUE format for PROP of MeasureGroupBrainPF_BS.
			% 
			...
			%   Error id: BRAPH2:MeasureGroupBrainPF_BS:WrongInput
			%  Element.CHECKPROP(MeasureGroupBrainPF_BS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of MeasureGroupBrainPF_BS.
			%   Error id: BRAPH2:MeasureGroupBrainPF_BS:WrongInput
			%  PF.CHECKPROP(MeasureGroupBrainPF_BS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of MeasureGroupBrainPF_BS.
			%   Error id: BRAPH2:MeasureGroupBrainPF_BS:WrongInput]
			% 
			% Note that the Element.CHECKPROP(PF) and Element.CHECKPROP('MeasureGroupBrainPF_BS')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 4 % MeasureGroupBrainPF_BS.TEMPLATE
					check = Format.checkFormat(8, value, MeasureGroupBrainPF_BS.getPropSettings(prop));
				otherwise
					if prop <= 47
			...
			elseif ~check
				error( ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput'], ...
					['BRAPH2' ':MeasureGroupBrainPF_BS:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' MeasureGroupBrainPF_BS.getPropTag(prop) ' (' MeasureGroupBrainPF_BS.getFormatTag(MeasureGroupBrainPF_BS.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
