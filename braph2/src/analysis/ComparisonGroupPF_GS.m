classdef ComparisonGroupPF_GS < ComparisonGroupPF
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = ComparisonGroupPF_GS.GETBUILD() returns the build of 'ComparisonGroupPF_GS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel global superglobal group comparison figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonGroupPF_GS') returns the build of 'ComparisonGroupPF_GS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonGroupPF_GS')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ComparisonGroupPF_GS' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF_GS:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF_GS:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				comparisongrouppf_gs_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'H_WAITBAR'  'DRAW'  'DRAWN'  'PARENT'  'BKGCOLOR'  'H'  'SHOW'  'HIDE'  'DELETE'  'CLOSE'  'ST_POSITION'  'H_TOOLBAR'  'H_TOOLS'  'H_AXES'  'ST_AXIS'  'LISTENER_ST_AXIS'  'CP'  'SETUP'  'H_AREA'  'ST_AREA'  'LISTENER_ST_AREA'  'H_LINE_DIFF'  'ST_LINE_DIFF'  'LISTENER_ST_LINE_DIFF'  'H_LINE_CIL'  'ST_LINE_CIL'  'LISTENER_ST_LINE_CIL'  'H_LINE_CIU'  'ST_LINE_CIU'  'LISTENER_ST_LINE_CIU'  'H_TITLE'  'ST_TITLE'  'H_XLABEL'  'ST_XLABEL'  'H_YLABEL'  'ST_YLABEL' };
				tag = comparisongrouppf_gs_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_gs_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8 };
			prop_category = comparisongrouppf_gs_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_gs_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8 };
			prop_format = comparisongrouppf_gs_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % ComparisonGroupPF_GS.TEMPLATE
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % ComparisonGroupPF_GS.ELCLASS
					prop_default = 'ComparisonGroupPF_GS';
				case 2 % ComparisonGroupPF_GS.NAME
					prop_default = 'ComparisonGroupPF_GS';
				case 3 % ComparisonGroupPF_GS.DESCRIPTION
				...
				case 4 % ComparisonGroupPF_GS.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonGroupPF_GS.getPropSettings(prop));
				case 5 % ComparisonGroupPF_GS.ID
					prop_default = 'ComparisonGroupPF_GS ID';
				case 6 % ComparisonGroupPF_GS.LABEL
					prop_default = 'ComparisonGroupPF_GS label';
				case 7 % ComparisonGroupPF_GS.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 4 % ComparisonGroupPF_GS.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonGroupPF_GS.getPropSettings(prop));
				otherwise
					if prop <= 45
