classdef ComparisonGroupPF_NS < ComparisonGroupPF
	%ComparisonGroupPF_NS is the base element to plot a nodal superglobal group comparison figure.
	...
	%
	% The list of ComparisonGroupPF_NS properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ComparisonGroupPF_NS.
	...
	%
	% ComparisonGroupPF_NS methods (constructor):
	%  ComparisonGroupPF_NS - constructor
	%
	% ComparisonGroupPF_NS methods:
	...
	%
	% ComparisonGroupPF_NS methods (display):
	...
	%
	% ComparisonGroupPF_NS methods (miscellanea):
	...
	%
	% ComparisonGroupPF_NS methods (save/load, Static):
	...
	%
	% ComparisonGroupPF_NS methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% ComparisonGroupPF_NS methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% ComparisonGroupPF_NS methods (category, Static):
	...
	%
	% ComparisonGroupPF_NS methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?ComparisonGroupPF_NS; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">ComparisonGroupPF_NS constants</a>.
	%
	%
	...
	methods % constructor
		function pf = ComparisonGroupPF_NS(varargin)
			%ComparisonGroupPF_NS() creates a panel nodal superglobal group comparison figure.
			%
			% ComparisonGroupPF_NS(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% ComparisonGroupPF_NS(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = ComparisonGroupPF_NS.GETBUILD() returns the build of 'ComparisonGroupPF_NS'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel nodal superglobal group comparison figure PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('ComparisonGroupPF_NS') returns the build of 'ComparisonGroupPF_NS'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('ComparisonGroupPF_NS')
		...
		function pf_class = getClass()
			%GETCLASS returns the class of the panel nodal superglobal group comparison figure.
			%
			% CLASS = ComparisonGroupPF_NS.GETCLASS() returns the class 'ComparisonGroupPF_NS'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PF.GETCLASS() returns the class of the panel nodal superglobal group comparison figure PF.
			%  CLASS = Element.GETCLASS(PF) returns the class of 'PF'.
			%  CLASS = Element.GETCLASS('ComparisonGroupPF_NS') returns 'ComparisonGroupPF_NS'.
			%
			% Note that the Element.GETCLASS(PF) and Element.GETCLASS('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			
			pf_class = 'ComparisonGroupPF_NS';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel nodal superglobal group comparison figure.
			%
			% LIST = ComparisonGroupPF_NS.GETSUBCLASSES() returns all subclasses of 'ComparisonGroupPF_NS'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel nodal superglobal group comparison figure PF.
			%  LIST = Element.GETSUBCLASSES(PF) returns all subclasses of 'PF'.
			%  LIST = Element.GETSUBCLASSES('ComparisonGroupPF_NS') returns all subclasses of 'ComparisonGroupPF_NS'.
			%
			% Note that the Element.GETSUBCLASSES(PF) and Element.GETSUBCLASSES('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'ComparisonGroupPF_NS' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel nodal superglobal group comparison figure.
			%
			% PROPS = ComparisonGroupPF_NS.GETPROPS() returns the property list of panel nodal superglobal group comparison figure
			%  as a row vector.
			%
			% PROPS = ComparisonGroupPF_NS.GETPROPS(CATEGORY) returns the property list 
			...
			%  PROPS = Element.GETPROPS('ComparisonGroupPF_NS'[, CATEGORY]) returns the property list of 'ComparisonGroupPF_NS'.
			%
			% Note that the Element.GETPROPS(PF) and Element.GETPROPS('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = ComparisonGroupPF_NS.GETPROPNUMBER() returns the property number of panel nodal superglobal group comparison figure.
			%
			% N = ComparisonGroupPF_NS.GETPROPNUMBER(CATEGORY) returns the property number of panel nodal superglobal group comparison figure
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			...
			%  N = Element.GETPROPNUMBER('ComparisonGroupPF_NS') returns the property number of 'ComparisonGroupPF_NS'.
			%
			% Note that the Element.GETPROPNUMBER(PF) and Element.GETPROPNUMBER('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%
			% CHECK = ComparisonGroupPF_NS.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSPROP(PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(PF, PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(ComparisonGroupPF_NS, PROP) checks whether PROP exists for ComparisonGroupPF_NS.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:ComparisonGroupPF_NS:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:ComparisonGroupPF_NS:WrongInput]
			%  Element.EXISTSPROP(ComparisonGroupPF_NS, PROP) throws error if PROP does NOT exist for ComparisonGroupPF_NS.
			%   Error id: [BRAPH2:ComparisonGroupPF_NS:WrongInput]
			%
			% Note that the Element.EXISTSPROP(PF) and Element.EXISTSPROP('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput' '\n' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for ComparisonGroupPF_NS.'] ...
					)
			end
		end
		function check_out = existsTag(tag)
			...
			%
			% CHECK = ComparisonGroupPF_NS.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSTAG(TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(PF, TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(ComparisonGroupPF_NS, TAG) checks whether TAG exists for ComparisonGroupPF_NS.
			%
			...
			%   Error id: [BRAPH2:ComparisonGroupPF_NS:WrongInput]
			%  Element.EXISTSTAG(ComparisonGroupPF_NS, TAG) throws error if TAG does NOT exist for ComparisonGroupPF_NS.
			%   Error id: [BRAPH2:ComparisonGroupPF_NS:WrongInput]
			%
			% Note that the Element.EXISTSTAG(PF) and Element.EXISTSTAG('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput' '\n' ...
					'The value ' tag ' is not a valid tag for ComparisonGroupPF_NS.'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(ComparisonGroupPF_NS, POINTER) returns property number of POINTER of ComparisonGroupPF_NS.
			%  PROPERTY = PF.GETPROPPROP(ComparisonGroupPF_NS, POINTER) returns property number of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPPROP(PF) and Element.GETPROPPROP('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(ComparisonGroupPF_NS, POINTER) returns tag of POINTER of ComparisonGroupPF_NS.
			%  TAG = PF.GETPROPTAG(ComparisonGroupPF_NS, POINTER) returns tag of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPTAG(PF) and Element.GETPROPTAG('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(ComparisonGroupPF_NS, POINTER) returns category of POINTER of ComparisonGroupPF_NS.
			%  CATEGORY = PF.GETPROPCATEGORY(ComparisonGroupPF_NS, POINTER) returns category of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPCATEGORY(PF) and Element.GETPROPCATEGORY('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			%  getPropFormat, getPropDescription, getPropDefault, checkProp.
			
			prop = ComparisonGroupPF_NS.getPropProp(pointer);
			
			%CET: Computational Efficiency Trick
			comparisongrouppf_ns_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  7  2  6  7  8  7  7  8  7  7  8  7  7  8  7  7  8  7  8  7  8  8 };
			prop_category = comparisongrouppf_ns_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(ComparisonGroupPF_NS, POINTER) returns format of POINTER of ComparisonGroupPF_NS.
			%  FORMAT = PF.GETPROPFORMAT(ComparisonGroupPF_NS, POINTER) returns format of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPFORMAT(PF) and Element.GETPROPFORMAT('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			comparisongrouppf_ns_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  8  18  8  1  18  8  18  18  8  18  18  8  18  18  8  18  18  8  18  8  18  8  11 };
			prop_format = comparisongrouppf_ns_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(ComparisonGroupPF_NS, POINTER) returns description of POINTER of ComparisonGroupPF_NS.
			%  DESCRIPTION = PF.GETPROPDESCRIPTION(ComparisonGroupPF_NS, POINTER) returns description of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPDESCRIPTION(PF) and Element.GETPROPDESCRIPTION('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(ComparisonGroupPF_NS, POINTER) returns settings of POINTER of ComparisonGroupPF_NS.
			%  SETTINGS = PF.GETPROPSETTINGS(ComparisonGroupPF_NS, POINTER) returns settings of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPSETTINGS(PF) and Element.GETPROPSETTINGS('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonGroupPF_NS.NODE
					prop_settings = Format.getFormatSettings(11);
				case 4 % ComparisonGroupPF_NS.TEMPLATE
					prop_settings = 'ComparisonGroupPF_NS';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = ComparisonGroupPF_NS.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = ComparisonGroupPF_NS.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULT(ComparisonGroupPF_NS, POINTER) returns the default value of POINTER of ComparisonGroupPF_NS.
			%  DEFAULT = PF.GETPROPDEFAULT(ComparisonGroupPF_NS, POINTER) returns the default value of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPDEFAULT(PF) and Element.GETPROPDEFAULT('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 46 % ComparisonGroupPF_NS.NODE
					prop_default = Format.getFormatDefault(11, ComparisonGroupPF_NS.getPropSettings(prop));
				case 1 % ComparisonGroupPF_NS.ELCLASS
					prop_default = 'ComparisonGroupPF_NS';
				case 2 % ComparisonGroupPF_NS.NAME
					prop_default = 'ComparisonGroupPF_NS';
				case 3 % ComparisonGroupPF_NS.DESCRIPTION
					prop_default = 'ComparisonGroupPF_NS manages the basic functionalities to plot of a nodal superglobal group comparison figure.';
				case 4 % ComparisonGroupPF_NS.TEMPLATE
					prop_default = Format.getFormatDefault(8, ComparisonGroupPF_NS.getPropSettings(prop));
				case 5 % ComparisonGroupPF_NS.ID
					prop_default = 'ComparisonGroupPF_NS ID';
				case 6 % ComparisonGroupPF_NS.LABEL
					prop_default = 'ComparisonGroupPF_NS label';
				case 7 % ComparisonGroupPF_NS.NOTES
					prop_default = 'ComparisonGroupPF_NS notes';
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = ComparisonGroupPF_NS.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = ComparisonGroupPF_NS.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(ComparisonGroupPF_NS, POINTER) returns the conditioned default value of POINTER of ComparisonGroupPF_NS.
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(ComparisonGroupPF_NS, POINTER) returns the conditioned default value of POINTER of ComparisonGroupPF_NS.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(PF) and Element.GETPROPDEFAULTCONDITIONED('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			prop_default = ComparisonGroupPF_NS.conditioning(prop, ComparisonGroupPF_NS.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(ComparisonGroupPF_NS, PROP, VALUE) checks VALUE format for PROP of ComparisonGroupPF_NS.
			%  CHECK = PF.CHECKPROP(ComparisonGroupPF_NS, PROP, VALUE) checks VALUE format for PROP of ComparisonGroupPF_NS.
			% 
			...
			%   Error id: BRAPH2:ComparisonGroupPF_NS:WrongInput
			%  Element.CHECKPROP(ComparisonGroupPF_NS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of ComparisonGroupPF_NS.
			%   Error id: BRAPH2:ComparisonGroupPF_NS:WrongInput
			%  PF.CHECKPROP(ComparisonGroupPF_NS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of ComparisonGroupPF_NS.
			%   Error id: BRAPH2:ComparisonGroupPF_NS:WrongInput]
			% 
			% Note that the Element.CHECKPROP(PF) and Element.CHECKPROP('ComparisonGroupPF_NS')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 46 % ComparisonGroupPF_NS.NODE
					check = Format.checkFormat(11, value, ComparisonGroupPF_NS.getPropSettings(prop));
				case 4 % ComparisonGroupPF_NS.TEMPLATE
					check = Format.checkFormat(8, value, ComparisonGroupPF_NS.getPropSettings(prop));
				otherwise
					if prop <= 45
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput'], ...
					['BRAPH2' ':ComparisonGroupPF_NS:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ComparisonGroupPF_NS.getPropTag(prop) ' (' ComparisonGroupPF_NS.getFormatTag(ComparisonGroupPF_NS.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 27 % ComparisonGroupPF_NS.SETUP
					cp = pf.get('CP');
