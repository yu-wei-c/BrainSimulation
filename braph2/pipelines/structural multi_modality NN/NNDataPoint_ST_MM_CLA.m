classdef NNDataPoint_ST_MM_CLA < NNDataPoint
	%NNDataPoint_ST_MM_CLA is a data point for classification with structural multimodality data.
	...
	%
	% NNDataPoint_ST_MM_CLA methods (display):
	%  tostring - string with information about the structural multimodality classification data point
	%  disp - displays information about the structural multimodality classification data point
	%  tree - displays the tree of the structural multimodality classification data point
	%
	% NNDataPoint_ST_MM_CLA methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two structural multimodality classification data point are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the structural multimodality classification data point
	%
	% NNDataPoint_ST_MM_CLA methods (save/load, Static):
	%  save - saves BRAPH2 structural multimodality classification data point as b2 file
	%  load - loads a BRAPH2 structural multimodality classification data point from a b2 file
	%
	% NNDataPoint_ST_MM_CLA method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the structural multimodality classification data point
	%
	% NNDataPoint_ST_MM_CLA method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the structural multimodality classification data point
	%
	% NNDataPoint_ST_MM_CLA methods (inspection, Static):
	%  getClass - returns the class of the structural multimodality classification data point
	%  getSubclasses - returns all subclasses of NNDataPoint_ST_MM_CLA
	%  getProps - returns the property list of the structural multimodality classification data point
	%  getPropNumber - returns the property number of the structural multimodality classification data point
	...
	%
	% NNDataPoint_ST_MM_CLA methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% NNDataPoint_ST_MM_CLA methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% NNDataPoint_ST_MM_CLA methods (category, Static):
	...
	%
	% NNDataPoint_ST_MM_CLA methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?NNDataPoint_ST_MM_CLA; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">NNDataPoint_ST_MM_CLA constants</a>.
	%
	%
	...
	methods % constructor
		function dp = NNDataPoint_ST_MM_CLA(varargin)
			%NNDataPoint_ST_MM_CLA() creates a structural multimodality classification data point.
			%
			% NNDataPoint_ST_MM_CLA(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% NNDataPoint_ST_MM_CLA(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function dp_class = getClass()
			%GETCLASS returns the class of the structural multimodality classification data point.
			%
			% CLASS = NNDataPoint_ST_MM_CLA.GETCLASS() returns the class 'NNDataPoint_ST_MM_CLA'.
			%
			% Alternative forms to call this method are:
			%  CLASS = DP.GETCLASS() returns the class of the structural multimodality classification data point DP.
			%  CLASS = Element.GETCLASS(DP) returns the class of 'DP'.
			%  CLASS = Element.GETCLASS('NNDataPoint_ST_MM_CLA') returns 'NNDataPoint_ST_MM_CLA'.
			%
			% Note that the Element.GETCLASS(DP) and Element.GETCLASS('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			
			dp_class = 'NNDataPoint_ST_MM_CLA';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the structural multimodality classification data point.
			%
			% LIST = NNDataPoint_ST_MM_CLA.GETSUBCLASSES() returns all subclasses of 'NNDataPoint_ST_MM_CLA'.
			%
			% Alternative forms to call this method are:
			%  LIST = DP.GETSUBCLASSES() returns all subclasses of the structural multimodality classification data point DP.
			%  LIST = Element.GETSUBCLASSES(DP) returns all subclasses of 'DP'.
			%  LIST = Element.GETSUBCLASSES('NNDataPoint_ST_MM_CLA') returns all subclasses of 'NNDataPoint_ST_MM_CLA'.
			%
			% Note that the Element.GETSUBCLASSES(DP) and Element.GETSUBCLASSES('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
			% See also subclasses.
		...
		function prop_list = getProps(category)
			...
			%
			% PROPS = NNDataPoint_ST_MM_CLA.GETPROPS() returns the property list of structural multimodality classification data point
			%  as a row vector.
			%
			% PROPS = NNDataPoint_ST_MM_CLA.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = DP.GETPROPS([CATEGORY]) returns the property list of the structural multimodality classification data point DP.
			%  PROPS = Element.GETPROPS(DP[, CATEGORY]) returns the property list of 'DP'.
			%  PROPS = Element.GETPROPS('NNDataPoint_ST_MM_CLA'[, CATEGORY]) returns the property list of 'NNDataPoint_ST_MM_CLA'.
			%
			% Note that the Element.GETPROPS(DP) and Element.GETPROPS('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of structural multimodality classification data point.
			%
			% N = NNDataPoint_ST_MM_CLA.GETPROPNUMBER() returns the property number of structural multimodality classification data point.
			%
			% N = NNDataPoint_ST_MM_CLA.GETPROPNUMBER(CATEGORY) returns the property number of structural multimodality classification data point
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = DP.GETPROPNUMBER([CATEGORY]) returns the property number of the structural multimodality classification data point DP.
			%  N = Element.GETPROPNUMBER(DP) returns the property number of 'DP'.
			%  N = Element.GETPROPNUMBER('NNDataPoint_ST_MM_CLA') returns the property number of 'NNDataPoint_ST_MM_CLA'.
			%
			% Note that the Element.GETPROPNUMBER(DP) and Element.GETPROPNUMBER('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%
			% CHECK = NNDataPoint_ST_MM_CLA.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			...
			%  CHECK = Element.EXISTSPROP(NNDataPoint_ST_MM_CLA, PROP) checks whether PROP exists for NNDataPoint_ST_MM_CLA.
			%
			...
			%   Error id: [BRAPH2:NNDataPoint_ST_MM_CLA:WrongInput]
			%  Element.EXISTSPROP(NNDataPoint_ST_MM_CLA, PROP) throws error if PROP does NOT exist for NNDataPoint_ST_MM_CLA.
			%   Error id: [BRAPH2:NNDataPoint_ST_MM_CLA:WrongInput]
			%
			% Note that the Element.EXISTSPROP(DP) and Element.EXISTSPROP('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in structural multimodality classification data point/error.
			%
			% CHECK = NNDataPoint_ST_MM_CLA.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = DP.EXISTSTAG(TAG) checks whether TAG exists for DP.
			%  CHECK = Element.EXISTSTAG(DP, TAG) checks whether TAG exists for DP.
			%  CHECK = Element.EXISTSTAG(NNDataPoint_ST_MM_CLA, TAG) checks whether TAG exists for NNDataPoint_ST_MM_CLA.
			%
			...
			%   Error id: [BRAPH2:NNDataPoint_ST_MM_CLA:WrongInput]
			%  Element.EXISTSTAG(NNDataPoint_ST_MM_CLA, TAG) throws error if TAG does NOT exist for NNDataPoint_ST_MM_CLA.
			%   Error id: [BRAPH2:NNDataPoint_ST_MM_CLA:WrongInput]
			%
			% Note that the Element.EXISTSTAG(DP) and Element.EXISTSTAG('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(NNDataPoint_ST_MM_CLA, POINTER) returns property number of POINTER of NNDataPoint_ST_MM_CLA.
			%  PROPERTY = DP.GETPROPPROP(NNDataPoint_ST_MM_CLA, POINTER) returns property number of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPPROP(DP) and Element.GETPROPPROP('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(NNDataPoint_ST_MM_CLA, POINTER) returns tag of POINTER of NNDataPoint_ST_MM_CLA.
			%  TAG = DP.GETPROPTAG(NNDataPoint_ST_MM_CLA, POINTER) returns tag of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPTAG(DP) and Element.GETPROPTAG('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(NNDataPoint_ST_MM_CLA, POINTER) returns category of POINTER of NNDataPoint_ST_MM_CLA.
			%  CATEGORY = DP.GETPROPCATEGORY(NNDataPoint_ST_MM_CLA, POINTER) returns category of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPCATEGORY(DP) and Element.GETPROPCATEGORY('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(NNDataPoint_ST_MM_CLA, POINTER) returns format of POINTER of NNDataPoint_ST_MM_CLA.
			%  FORMAT = DP.GETPROPFORMAT(NNDataPoint_ST_MM_CLA, POINTER) returns format of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPFORMAT(DP) and Element.GETPROPFORMAT('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(NNDataPoint_ST_MM_CLA, POINTER) returns description of POINTER of NNDataPoint_ST_MM_CLA.
			%  DESCRIPTION = DP.GETPROPDESCRIPTION(NNDataPoint_ST_MM_CLA, POINTER) returns description of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPDESCRIPTION(DP) and Element.GETPROPDESCRIPTION('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(NNDataPoint_ST_MM_CLA, POINTER) returns settings of POINTER of NNDataPoint_ST_MM_CLA.
			%  SETTINGS = DP.GETPROPSETTINGS(NNDataPoint_ST_MM_CLA, POINTER) returns settings of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPSETTINGS(DP) and Element.GETPROPSETTINGS('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = NNDataPoint_ST_MM_CLA.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = NNDataPoint_ST_MM_CLA.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = DP.GETPROPDEFAULT(POINTER) returns the default value of POINTER of DP.
			%  DEFAULT = Element.GETPROPDEFAULT(NNDataPoint_ST_MM_CLA, POINTER) returns the default value of POINTER of NNDataPoint_ST_MM_CLA.
			%  DEFAULT = DP.GETPROPDEFAULT(NNDataPoint_ST_MM_CLA, POINTER) returns the default value of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPDEFAULT(DP) and Element.GETPROPDEFAULT('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = NNDataPoint_ST_MM_CLA.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = NNDataPoint_ST_MM_CLA.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = DP.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of DP.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(NNDataPoint_ST_MM_CLA, POINTER) returns the conditioned default value of POINTER of NNDataPoint_ST_MM_CLA.
			%  DEFAULT = DP.GETPROPDEFAULTCONDITIONED(NNDataPoint_ST_MM_CLA, POINTER) returns the conditioned default value of POINTER of NNDataPoint_ST_MM_CLA.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(DP) and Element.GETPROPDEFAULTCONDITIONED('NNDataPoint_ST_MM_CLA')
			%  are less computationally efficient.
			%
			...
			prop_default = NNDataPoint_ST_MM_CLA.conditioning(prop, NNDataPoint_ST_MM_CLA.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(NNDataPoint_ST_MM_CLA, PROP, VALUE) checks VALUE format for PROP of NNDataPoint_ST_MM_CLA.
			%  CHECK = DP.CHECKPROP(NNDataPoint_ST_MM_CLA, PROP, VALUE) checks VALUE format for PROP of NNDataPoint_ST_MM_CLA.
			% 
