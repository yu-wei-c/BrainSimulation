classdef AnalyzeGroup_CON_FUN_MP_GA_WU < AnalyzeGroup
	...
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the graph analysis with connectivity and functional multiplex data.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the graph analysis with connectivity and functional multiplex data.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the graph analysis with connectivity and functional multiplex data.
	...
	%
	% AnalyzeGroup_CON_FUN_MP_GA_WU methods (miscellanea):
	...
	%
	% AnalyzeGroup_CON_FUN_MP_GA_WU methods (save/load, Static):
	...
	%  getSubclasses - returns all subclasses of AnalyzeGroup_CON_FUN_MP_GA_WU
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?AnalyzeGroup_CON_FUN_MP_GA_WU; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">AnalyzeGroup_CON_FUN_MP_GA_WU constants</a>.
	%
	%
	...
	methods % constructor
		function a = AnalyzeGroup_CON_FUN_MP_GA_WU(varargin)
			...
			%
			% AnalyzeGroup_CON_FUN_MP_GA_WU(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function a_class = getClass()
			...
			%
			% Note that the Element.GETCLASS(A) and Element.GETCLASS('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
		...
		function subclass_list = getSubclasses()
			...
			%
			% LIST = AnalyzeGroup_CON_FUN_MP_GA_WU.GETSUBCLASSES() returns all subclasses of 'AnalyzeGroup_CON_FUN_MP_GA_WU'.
			%
			% Alternative forms to call this method are:
			...
			%  LIST = Element.GETSUBCLASSES('AnalyzeGroup_CON_FUN_MP_GA_WU') returns all subclasses of 'AnalyzeGroup_CON_FUN_MP_GA_WU'.
			%
			% Note that the Element.GETSUBCLASSES(A) and Element.GETSUBCLASSES('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'AnalyzeGroup_CON_FUN_MP_GA_WU' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			...
			%
			% Note that the Element.GETPROPS(A) and Element.GETPROPS('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = AnalyzeGroup_CON_FUN_MP_GA_WU.GETPROPNUMBER(CATEGORY) returns the property number of graph analysis with connectivity and functional multiplex data
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			...
			%
			% Note that the Element.GETPROPNUMBER(A) and Element.GETPROPNUMBER('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_CON_FUN_MP_GA_WU:WrongInput]
			%
			% Note that the Element.EXISTSPROP(A) and Element.EXISTSPROP('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_CON_FUN_MP_GA_WU:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_CON_FUN_MP_GA_WU:' 'WrongInput' '\n' ...
		...
		function check_out = existsTag(tag)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_CON_FUN_MP_GA_WU:WrongInput]
			%
			% Note that the Element.EXISTSTAG(A) and Element.EXISTSTAG('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%
			% Note that the Element.GETPROPPROP(A) and Element.GETPROPPROP('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%
			% Note that the Element.GETPROPTAG(A) and Element.GETPROPTAG('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				analyzegroup_con_fun_mp_ga_wu_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'GR'  'G'  'REPETITION'  'F_MIN'  'F_MAX'  'CORRELATION_RULE'  'NEGATIVE_WEIGHT_RULE' };
				tag = analyzegroup_con_fun_mp_ga_wu_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%
			% Note that the Element.GETPROPCATEGORY(A) and Element.GETPROPCATEGORY('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzegroup_con_fun_mp_ga_wu_category_list = { 1  1  1  3  4  2  2  6  9  4  5  3  3  3  3  3 };
			prop_category = analyzegroup_con_fun_mp_ga_wu_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%
			% Note that the Element.GETPROPFORMAT(A) and Element.GETPROPFORMAT('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzegroup_con_fun_mp_ga_wu_format_list = { 2  2  2  8  2  2  2  2  4  8  8  11  11  11  5  5 };
			prop_format = analyzegroup_con_fun_mp_ga_wu_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%
			% Note that the Element.GETPROPDESCRIPTION(A) and Element.GETPROPDESCRIPTION('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%
			% Note that the Element.GETPROPSETTINGS(A) and Element.GETPROPSETTINGS('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 12 % AnalyzeGroup_CON_FUN_MP_GA_WU.REPETITION
					prop_settings = Format.getFormatSettings(11);
				case 13 % AnalyzeGroup_CON_FUN_MP_GA_WU.F_MIN
					prop_settings = Format.getFormatSettings(11);
				case 14 % AnalyzeGroup_CON_FUN_MP_GA_WU.F_MAX
					prop_settings = Format.getFormatSettings(11);
				case 15 % AnalyzeGroup_CON_FUN_MP_GA_WU.CORRELATION_RULE
					prop_settings = Correlation.CORRELATION_RULE_LIST(1:3);
				case 16 % AnalyzeGroup_CON_FUN_MP_GA_WU.NEGATIVE_WEIGHT_RULE
					prop_settings = Correlation.NEGATIVE_WEIGHT_RULE_LIST;
				case 4 % AnalyzeGroup_CON_FUN_MP_GA_WU.TEMPLATE
					prop_settings = 'AnalyzeGroup_CON_FUN_MP_GA_WU';
				case 11 % AnalyzeGroup_CON_FUN_MP_GA_WU.G
		...
		function prop_default = getPropDefault(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULT(A) and Element.GETPROPDEFAULT('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 4 % AnalyzeGroup_CON_FUN_MP_GA_WU.TEMPLATE
					prop_default = Format.getFormatDefault(8, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 5 % AnalyzeGroup_CON_FUN_MP_GA_WU.ID
					prop_default = 'AnalyzeGroup_CON_FUN_MP_GA_WU ID';
				case 6 % AnalyzeGroup_CON_FUN_MP_GA_WU.LABEL
					prop_default = 'AnalyzeGroup_CON_FUN_MP_GA_WU label';
				case 7 % AnalyzeGroup_CON_FUN_MP_GA_WU.NOTES
					prop_default = 'AnalyzeGroup_CON_FUN_MP_GA_WU notes';
				case 10 % AnalyzeGroup_CON_FUN_MP_GA_WU.GR
		...
		function prop_default = getPropDefaultConditioned(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(A) and Element.GETPROPDEFAULTCONDITIONED('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%   Error id: BRAPH2:AnalyzeGroup_CON_FUN_MP_GA_WU:WrongInput]
			% 
			% Note that the Element.CHECKPROP(A) and Element.CHECKPROP('AnalyzeGroup_CON_FUN_MP_GA_WU')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 12 % AnalyzeGroup_CON_FUN_MP_GA_WU.REPETITION
					check = Format.checkFormat(11, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 13 % AnalyzeGroup_CON_FUN_MP_GA_WU.F_MIN
					check = Format.checkFormat(11, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 14 % AnalyzeGroup_CON_FUN_MP_GA_WU.F_MAX
					check = Format.checkFormat(11, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 15 % AnalyzeGroup_CON_FUN_MP_GA_WU.CORRELATION_RULE
					check = Format.checkFormat(5, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 16 % AnalyzeGroup_CON_FUN_MP_GA_WU.NEGATIVE_WEIGHT_RULE
					check = Format.checkFormat(5, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 4 % AnalyzeGroup_CON_FUN_MP_GA_WU.TEMPLATE
					check = Format.checkFormat(8, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				case 11 % AnalyzeGroup_CON_FUN_MP_GA_WU.G
					check = Format.checkFormat(8, value, AnalyzeGroup_CON_FUN_MP_GA_WU.getPropSettings(prop));
				otherwise
					if prop <= 11
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_CON_FUN_MP_GA_WU:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_CON_FUN_MP_GA_WU:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeGroup_CON_FUN_MP_GA_WU.getPropTag(prop) ' (' AnalyzeGroup_CON_FUN_MP_GA_WU.getFormatTag(AnalyzeGroup_CON_FUN_MP_GA_WU.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
