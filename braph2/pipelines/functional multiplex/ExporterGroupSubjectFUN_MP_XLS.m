classdef ExporterGroupSubjectFUN_MP_XLS < Exporter
	%ExporterGroupSubjectFUN_MP_XLS exports a group of subjects with functional multiplex data to a series of XLSX file.
	...
	%
	% ExporterGroupSubjectFUN_MP_XLS methods:
	...
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (display):
	%  tostring - string with information about the exporter of FUN MP subject group in XLSX
	%  disp - displays information about the exporter of FUN MP subject group in XLSX
	%  tree - displays the tree of the exporter of FUN MP subject group in XLSX
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two exporter of FUN MP subject group in XLSX are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the exporter of FUN MP subject group in XLSX
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (save/load, Static):
	%  save - saves BRAPH2 exporter of FUN MP subject group in XLSX as b2 file
	%  load - loads a BRAPH2 exporter of FUN MP subject group in XLSX from a b2 file
	%
	% ExporterGroupSubjectFUN_MP_XLS method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the exporter of FUN MP subject group in XLSX
	%
	% ExporterGroupSubjectFUN_MP_XLS method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the exporter of FUN MP subject group in XLSX
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (inspection, Static):
	%  getClass - returns the class of the exporter of FUN MP subject group in XLSX
	%  getSubclasses - returns all subclasses of ExporterGroupSubjectFUN_MP_XLS
	%  getProps - returns the property list of the exporter of FUN MP subject group in XLSX
	%  getPropNumber - returns the property number of the exporter of FUN MP subject group in XLSX
	...
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (category, Static):
	...
	%
	% ExporterGroupSubjectFUN_MP_XLS methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?ExporterGroupSubjectFUN_MP_XLS; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">ExporterGroupSubjectFUN_MP_XLS constants</a>.
	%
	%
	...
	methods % constructor
		function ex = ExporterGroupSubjectFUN_MP_XLS(varargin)
			%ExporterGroupSubjectFUN_MP_XLS() creates a exporter of FUN MP subject group in XLSX.
			%
			% ExporterGroupSubjectFUN_MP_XLS(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% ExporterGroupSubjectFUN_MP_XLS(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function ex_class = getClass()
			%GETCLASS returns the class of the exporter of FUN MP subject group in XLSX.
			%
			% CLASS = ExporterGroupSubjectFUN_MP_XLS.GETCLASS() returns the class 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Alternative forms to call this method are:
			%  CLASS = EX.GETCLASS() returns the class of the exporter of FUN MP subject group in XLSX EX.
			%  CLASS = Element.GETCLASS(EX) returns the class of 'EX'.
			%  CLASS = Element.GETCLASS('ExporterGroupSubjectFUN_MP_XLS') returns 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Note that the Element.GETCLASS(EX) and Element.GETCLASS('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			
			ex_class = 'ExporterGroupSubjectFUN_MP_XLS';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the exporter of FUN MP subject group in XLSX.
			%
			% LIST = ExporterGroupSubjectFUN_MP_XLS.GETSUBCLASSES() returns all subclasses of 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Alternative forms to call this method are:
			%  LIST = EX.GETSUBCLASSES() returns all subclasses of the exporter of FUN MP subject group in XLSX EX.
			%  LIST = Element.GETSUBCLASSES(EX) returns all subclasses of 'EX'.
			%  LIST = Element.GETSUBCLASSES('ExporterGroupSubjectFUN_MP_XLS') returns all subclasses of 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Note that the Element.GETSUBCLASSES(EX) and Element.GETSUBCLASSES('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'ExporterGroupSubjectFUN_MP_XLS' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of exporter of FUN MP subject group in XLSX.
			%
			% PROPS = ExporterGroupSubjectFUN_MP_XLS.GETPROPS() returns the property list of exporter of FUN MP subject group in XLSX
			%  as a row vector.
			%
			% PROPS = ExporterGroupSubjectFUN_MP_XLS.GETPROPS(CATEGORY) returns the property list 
			%  of category CATEGORY.
			%
			% Alternative forms to call this method are:
			%  PROPS = EX.GETPROPS([CATEGORY]) returns the property list of the exporter of FUN MP subject group in XLSX EX.
			%  PROPS = Element.GETPROPS(EX[, CATEGORY]) returns the property list of 'EX'.
			%  PROPS = Element.GETPROPS('ExporterGroupSubjectFUN_MP_XLS'[, CATEGORY]) returns the property list of 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Note that the Element.GETPROPS(EX) and Element.GETPROPS('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of exporter of FUN MP subject group in XLSX.
			%
			% N = ExporterGroupSubjectFUN_MP_XLS.GETPROPNUMBER() returns the property number of exporter of FUN MP subject group in XLSX.
			%
			% N = ExporterGroupSubjectFUN_MP_XLS.GETPROPNUMBER(CATEGORY) returns the property number of exporter of FUN MP subject group in XLSX
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = EX.GETPROPNUMBER([CATEGORY]) returns the property number of the exporter of FUN MP subject group in XLSX EX.
			%  N = Element.GETPROPNUMBER(EX) returns the property number of 'EX'.
			%  N = Element.GETPROPNUMBER('ExporterGroupSubjectFUN_MP_XLS') returns the property number of 'ExporterGroupSubjectFUN_MP_XLS'.
			%
			% Note that the Element.GETPROPNUMBER(EX) and Element.GETPROPNUMBER('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in exporter of FUN MP subject group in XLSX/error.
			%
			% CHECK = ExporterGroupSubjectFUN_MP_XLS.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			...
			%  CHECK = Element.EXISTSPROP(ExporterGroupSubjectFUN_MP_XLS, PROP) checks whether PROP exists for ExporterGroupSubjectFUN_MP_XLS.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			%  Element.EXISTSPROP(ExporterGroupSubjectFUN_MP_XLS, PROP) throws error if PROP does NOT exist for ExporterGroupSubjectFUN_MP_XLS.
			%   Error id: [BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			%
			% Note that the Element.EXISTSPROP(EX) and Element.EXISTSPROP('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput'], ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput' '\n' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for ExporterGroupSubjectFUN_MP_XLS.'] ...
					)
			end
		end
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in exporter of FUN MP subject group in XLSX/error.
			%
			% CHECK = ExporterGroupSubjectFUN_MP_XLS.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = EX.EXISTSTAG(TAG) checks whether TAG exists for EX.
			%  CHECK = Element.EXISTSTAG(EX, TAG) checks whether TAG exists for EX.
			%  CHECK = Element.EXISTSTAG(ExporterGroupSubjectFUN_MP_XLS, TAG) checks whether TAG exists for ExporterGroupSubjectFUN_MP_XLS.
			%
			...
			%   Error id: [BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			%  Element.EXISTSTAG(ExporterGroupSubjectFUN_MP_XLS, TAG) throws error if TAG does NOT exist for ExporterGroupSubjectFUN_MP_XLS.
			%   Error id: [BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			%
			% Note that the Element.EXISTSTAG(EX) and Element.EXISTSTAG('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput'], ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput' '\n' ...
					'The value ' tag ' is not a valid tag for ExporterGroupSubjectFUN_MP_XLS.'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns property number of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  PROPERTY = EX.GETPROPPROP(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns property number of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPPROP(EX) and Element.GETPROPPROP('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns tag of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  TAG = EX.GETPROPTAG(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns tag of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPTAG(EX) and Element.GETPROPTAG('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns category of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  CATEGORY = EX.GETPROPCATEGORY(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns category of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPCATEGORY(EX) and Element.GETPROPCATEGORY('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns format of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  FORMAT = EX.GETPROPFORMAT(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns format of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPFORMAT(EX) and Element.GETPROPFORMAT('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns description of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  DESCRIPTION = EX.GETPROPDESCRIPTION(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns description of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPDESCRIPTION(EX) and Element.GETPROPDESCRIPTION('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns settings of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  SETTINGS = EX.GETPROPSETTINGS(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns settings of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPSETTINGS(EX) and Element.GETPROPSETTINGS('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = ExporterGroupSubjectFUN_MP_XLS.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = ExporterGroupSubjectFUN_MP_XLS.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = EX.GETPROPDEFAULT(POINTER) returns the default value of POINTER of EX.
			%  DEFAULT = Element.GETPROPDEFAULT(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns the default value of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  DEFAULT = EX.GETPROPDEFAULT(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns the default value of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPDEFAULT(EX) and Element.GETPROPDEFAULT('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = ExporterGroupSubjectFUN_MP_XLS.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = ExporterGroupSubjectFUN_MP_XLS.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = EX.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of EX.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns the conditioned default value of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%  DEFAULT = EX.GETPROPDEFAULTCONDITIONED(ExporterGroupSubjectFUN_MP_XLS, POINTER) returns the conditioned default value of POINTER of ExporterGroupSubjectFUN_MP_XLS.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(EX) and Element.GETPROPDEFAULTCONDITIONED('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
			...
			prop_default = ExporterGroupSubjectFUN_MP_XLS.conditioning(prop, ExporterGroupSubjectFUN_MP_XLS.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(ExporterGroupSubjectFUN_MP_XLS, PROP, VALUE) checks VALUE format for PROP of ExporterGroupSubjectFUN_MP_XLS.
			%  CHECK = EX.CHECKPROP(ExporterGroupSubjectFUN_MP_XLS, PROP, VALUE) checks VALUE format for PROP of ExporterGroupSubjectFUN_MP_XLS.
			% 
			...
			%   Error id: BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput
			%  Element.CHECKPROP(ExporterGroupSubjectFUN_MP_XLS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of ExporterGroupSubjectFUN_MP_XLS.
			%   Error id: BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput
			%  EX.CHECKPROP(ExporterGroupSubjectFUN_MP_XLS, PROP, VALUE) throws error if VALUE has not a valid format for PROP of ExporterGroupSubjectFUN_MP_XLS.
			%   Error id: BRAPH2:ExporterGroupSubjectFUN_MP_XLS:WrongInput]
			% 
			% Note that the Element.CHECKPROP(EX) and Element.CHECKPROP('ExporterGroupSubjectFUN_MP_XLS')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput'], ...
					['BRAPH2' ':ExporterGroupSubjectFUN_MP_XLS:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' ExporterGroupSubjectFUN_MP_XLS.getPropTag(prop) ' (' ExporterGroupSubjectFUN_MP_XLS.getFormatTag(ExporterGroupSubjectFUN_MP_XLS.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
