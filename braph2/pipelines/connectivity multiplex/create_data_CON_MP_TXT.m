function create_data_CON_MP_TXT(data_dir, random_seed)
%CREATE_DATA_CON_MP_TXT creates connectivity data
%
% CREATE_DATA_CON_MP_TXT() creates connectivity data in default folder 'Example
%  data CON MP TXT'.
%
% CREATE_DATA_CON_MP_TXT(DATA_DIR) creates connectivity data in DATA_DIR folder.
%
% CREATE_DATA_CON_MP_TXT(DATA_DIR, RANDOM_SEED) cretes connectivity data in
%  DATA_DIR folder with a specified RANDOM_SEED.
%
% See also create_data_con_mp_xls.

if nargin < 1
    data_dir = [fileparts(which('SubjectCON_MP')) filesep 'Example data CON_MP TXT'];
end
...
if ~isfolder(data_dir)
    ...
    for i = 1:1:25 % subject number
        ...
        writetable(array2table(A_full1), [gr1_dir filesep() sub_id '.1.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
        writetable(array2table(A_full2), [gr1_dir filesep() sub_id '.2.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
    ...
    for i = 26:1:50
        ...
        writetable(array2table(A_full1), [gr2_dir filesep() sub_id '.1.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
        writetable(array2table(A_full2), [gr2_dir filesep() sub_id '.2.txt'], 'Delimiter', '\t', 'WriteVariableNames', false)
