classdef ExporterGroupSubjectCON_MP_TXT < Exporter
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the CON MP subject group exporter in TXT.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the exporter of CON MP subject group in TXT.
			%
			% BUILD = ExporterGroupSubjectCON_MP_TXT.GETBUILD() returns the build of 'ExporterGroupSubjectCON_MP_TXT'.
			%
			% Alternative forms to call this method are:
			%  BUILD = EX.GETBUILD() returns the build of the exporter of CON MP subject group in TXT EX.
			%  BUILD = Element.GETBUILD(EX) returns the build of 'EX'.
			%  BUILD = Element.GETBUILD('ExporterGroupSubjectCON_MP_TXT') returns the build of 'ExporterGroupSubjectCON_MP_TXT'.
			%
			% Note that the Element.GETBUILD(EX) and Element.GETBUILD('ExporterGroupSubjectCON_MP_TXT')
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'ExporterGroupSubjectCON_MP_TXT' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':ExporterGroupSubjectCON_MP_TXT:' 'WrongInput'], ...
					['BRAPH2' ':ExporterGroupSubjectCON_MP_TXT:' 'WrongInput' '\n' ...
