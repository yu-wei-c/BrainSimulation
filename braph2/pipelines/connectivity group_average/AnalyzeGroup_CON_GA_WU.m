classdef AnalyzeGroup_CON_GA_WU < AnalyzeGroup
	...
	%  tostring - string with information about the graph analysis with averaged connectivity data
	%  disp - displays information about the graph analysis with averaged connectivity data
	%  tree - displays the tree of the graph analysis with averaged connectivity data
	%
	% AnalyzeGroup_CON_GA_WU methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two graph analysis with averaged connectivity data are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the graph analysis with averaged connectivity data
	%
	% AnalyzeGroup_CON_GA_WU methods (save/load, Static):
	%  save - saves BRAPH2 graph analysis with averaged connectivity data as b2 file
	%  load - loads a BRAPH2 graph analysis with averaged connectivity data from a b2 file
	%
	% AnalyzeGroup_CON_GA_WU method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the graph analysis with averaged connectivity data
	%
	% AnalyzeGroup_CON_GA_WU method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the graph analysis with averaged connectivity data
	%
	...
	%  getSubclasses - returns all subclasses of AnalyzeGroup_CON_GA_WU
	%  getProps - returns the property list of the graph analysis with averaged connectivity data
	%  getPropNumber - returns the property number of the graph analysis with averaged connectivity data
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?AnalyzeGroup_CON_GA_WU; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">AnalyzeGroup_CON_GA_WU constants</a>.
	%
	%
	...
	methods % constructor
		function a = AnalyzeGroup_CON_GA_WU(varargin)
			...
			%
			% AnalyzeGroup_CON_GA_WU(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function a_class = getClass()
			...
			%
			% Note that the Element.GETCLASS(A) and Element.GETCLASS('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			
			a_class = 'AnalyzeGroup_CON_GA_WU';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the graph analysis with averaged connectivity data.
			%
			% LIST = AnalyzeGroup_CON_GA_WU.GETSUBCLASSES() returns all subclasses of 'AnalyzeGroup_CON_GA_WU'.
			%
			% Alternative forms to call this method are:
			%  LIST = A.GETSUBCLASSES() returns all subclasses of the graph analysis with averaged connectivity data A.
			%  LIST = Element.GETSUBCLASSES(A) returns all subclasses of 'A'.
			%  LIST = Element.GETSUBCLASSES('AnalyzeGroup_CON_GA_WU') returns all subclasses of 'AnalyzeGroup_CON_GA_WU'.
			%
			% Note that the Element.GETSUBCLASSES(A) and Element.GETSUBCLASSES('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
			% See also subclasses.
		...
		function prop_list = getProps(category)
			...
			%
			% Note that the Element.GETPROPS(A) and Element.GETPROPS('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = AnalyzeGroup_CON_GA_WU.GETPROPNUMBER(CATEGORY) returns the property number of graph analysis with averaged connectivity data
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER([CATEGORY]) returns the property number of the graph analysis with averaged connectivity data A.
			...
			%
			% Note that the Element.GETPROPNUMBER(A) and Element.GETPROPNUMBER('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_CON_GA_WU:WrongInput]
			%
			% Note that the Element.EXISTSPROP(A) and Element.EXISTSPROP('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsTag(tag)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_CON_GA_WU:WrongInput]
			%
			% Note that the Element.EXISTSTAG(A) and Element.EXISTSTAG('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%
			% Note that the Element.GETPROPPROP(A) and Element.GETPROPPROP('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%
			% Note that the Element.GETPROPTAG(A) and Element.GETPROPTAG('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%
			% Note that the Element.GETPROPCATEGORY(A) and Element.GETPROPCATEGORY('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_format = getPropFormat(pointer)
			...
			%
			% Note that the Element.GETPROPFORMAT(A) and Element.GETPROPFORMAT('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%
			% Note that the Element.GETPROPDESCRIPTION(A) and Element.GETPROPDESCRIPTION('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%
			% Note that the Element.GETPROPSETTINGS(A) and Element.GETPROPSETTINGS('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULT(A) and Element.GETPROPDEFAULT('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(A) and Element.GETPROPDEFAULTCONDITIONED('AnalyzeGroup_CON_GA_WU')
			%  are less computationally efficient.
			%
