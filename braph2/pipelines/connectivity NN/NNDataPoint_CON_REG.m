classdef NNDataPoint_CON_REG < NNDataPoint
	...
	%
	% The list of NNDataPoint_CON_REG properties is:
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the % % % .
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of a data point for regression with connectivity data.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of a data point for regression with connectivity data.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of a data point for regression with connectivity data.
	...
	methods (Static) % inspection
		...
		function subclass_list = getSubclasses()
			...
			subclass_list = { 'NNDataPoint_CON_REG' }; %CET: Computational Efficiency Trick
		end
		...
		function check_out = existsProp(prop)
			...
			elseif ~check
				error( ...
					['BRAPH2' ':NNDataPoint_CON_REG:' 'WrongInput'], ...
					['BRAPH2' ':NNDataPoint_CON_REG:' 'WrongInput' '\n' ...
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				nndatapoint_con_reg_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'INPUT'  'TARGET'  'SUB'  'TARGET_IDS' };
				tag = nndatapoint_con_reg_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			nndatapoint_con_reg_category_list = { 1  1  1  3  4  2  2  6  5  5  4  3 };
			prop_category = nndatapoint_con_reg_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			nndatapoint_con_reg_format_list = { 2  2  2  8  2  2  2  2  16  16  8  3 };
			prop_format = nndatapoint_con_reg_format_list{prop};
		end
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 11 % NNDataPoint_CON_REG.SUB
					prop_settings = 'SubjectCON';
				case 12 % NNDataPoint_CON_REG.TARGET_IDS
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 11 % NNDataPoint_CON_REG.SUB
					prop_default = Format.getFormatDefault(8, NNDataPoint_CON_REG.getPropSettings(prop));
				case 12 % NNDataPoint_CON_REG.TARGET_IDS
					prop_default = Format.getFormatDefault(3, NNDataPoint_CON_REG.getPropSettings(prop));
				...
				case 4 % NNDataPoint_CON_REG.TEMPLATE
					prop_default = Format.getFormatDefault(8, NNDataPoint_CON_REG.getPropSettings(prop));
				case 5 % NNDataPoint_CON_REG.ID
					prop_default = 'NNDataPoint_CON_REG ID';
				case 6 % NNDataPoint_CON_REG.LABEL
					prop_default = 'NNDataPoint_CON_REG label';
				case 7 % NNDataPoint_CON_REG.NOTES
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			switch prop
				case 11 % NNDataPoint_CON_REG.SUB
					check = Format.checkFormat(8, value, NNDataPoint_CON_REG.getPropSettings(prop));
				case 12 % NNDataPoint_CON_REG.TARGET_IDS
					check = Format.checkFormat(3, value, NNDataPoint_CON_REG.getPropSettings(prop));
				case 4 % NNDataPoint_CON_REG.TEMPLATE
					check = Format.checkFormat(8, value, NNDataPoint_CON_REG.getPropSettings(prop));
				otherwise
					if prop <= 10
	...
	methods (Access=protected) % calculate value
		function value = calculateValue(dp, prop, varargin)
			...
			switch prop
				case 9 % NNDataPoint_CON_REG.INPUT
				...
				case 10 % NNDataPoint_CON_REG.TARGET
