classdef NNDataPoint_ST_CLA < NNDataPoint
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the data point for classification with structural data.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the structural classification data point.
			%
			% BUILD = NNDataPoint_ST_CLA.GETBUILD() returns the build of 'NNDataPoint_ST_CLA'.
			%
			% Alternative forms to call this method are:
			%  BUILD = DP.GETBUILD() returns the build of the structural classification data point DP.
			%  BUILD = Element.GETBUILD(DP) returns the build of 'DP'.
			%  BUILD = Element.GETBUILD('NNDataPoint_ST_CLA') returns the build of 'NNDataPoint_ST_CLA'.
			%
			% Note that the Element.GETBUILD(DP) and Element.GETBUILD('NNDataPoint_ST_CLA')
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				nndatapoint_st_cla_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'INPUT'  'TARGET'  'SUB'  'TARGET_CLASS' };
		...
		function prop_description = getPropDescription(pointer)
			...
			%CET: Computational Efficiency Trick
			nndatapoint_st_cla_description_list = { 'ELCLASS (constant, string) is the class of the data point for classification with structural data.'  'NAME (constant, string) is the name of a data point for classification with structural data.'  'DESCRIPTION (constant, string) is the description of a data point for classification with structural data.'  'TEMPLATE (parameter, item) is the template of a data point for classification with structural data.'  'ID (data, string) is a few-letter code for a data point for classification with structural data.'  'LABEL (metadata, string) is an extended label of a data point for classification with structural data.'  'NOTES (metadata, string) are some specific notes about a data point for classification with structural data.'  'TOSTRING (query, string) returns a string that represents the concrete element.'  'INPUT (result, cell) is the input value for this data point.'  'TARGET (result, cell) is the target values for this data point.'  'SUB (data, item) is a subject with structural data.'  'TARGET_CLASS (parameter, stringlist) is a list of variable-of-interest IDs to be used as the class targets.' };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 12 % NNDataPoint_ST_CLA.TARGET_CLASS
