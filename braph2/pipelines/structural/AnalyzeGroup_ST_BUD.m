classdef AnalyzeGroup_ST_BUD < AnalyzeGroup
	%AnalyzeGroup_ST_BUD is a graph analysis using structural data at fixed density.
	...
	%  tostring - string with information about the graph analysis with structural data at fixed density
	%  disp - displays information about the graph analysis with structural data at fixed density
	%  tree - displays the tree of the graph analysis with structural data at fixed density
	%
	% AnalyzeGroup_ST_BUD methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two graph analysis with structural data at fixed density are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the graph analysis with structural data at fixed density
	%
	% AnalyzeGroup_ST_BUD methods (save/load, Static):
	%  save - saves BRAPH2 graph analysis with structural data at fixed density as b2 file
	%  load - loads a BRAPH2 graph analysis with structural data at fixed density from a b2 file
	%
	% AnalyzeGroup_ST_BUD method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the graph analysis with structural data at fixed density
	%
	% AnalyzeGroup_ST_BUD method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the graph analysis with structural data at fixed density
	%
	...
	%  getSubclasses - returns all subclasses of AnalyzeGroup_ST_BUD
	%  getProps - returns the property list of the graph analysis with structural data at fixed density
	%  getPropNumber - returns the property number of the graph analysis with structural data at fixed density
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?AnalyzeGroup_ST_BUD; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">AnalyzeGroup_ST_BUD constants</a>.
	%
	%
	...
	methods % constructor
		function a = AnalyzeGroup_ST_BUD(varargin)
			%AnalyzeGroup_ST_BUD() creates a graph analysis with structural data at fixed density.
			%
			% AnalyzeGroup_ST_BUD(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% AnalyzeGroup_ST_BUD(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function a_class = getClass()
			...
			%
			% Alternative forms to call this method are:
			%  CLASS = A.GETCLASS() returns the class of the graph analysis with structural data at fixed density A.
			%  CLASS = Element.GETCLASS(A) returns the class of 'A'.
			%  CLASS = Element.GETCLASS('AnalyzeGroup_ST_BUD') returns 'AnalyzeGroup_ST_BUD'.
			%
			% Note that the Element.GETCLASS(A) and Element.GETCLASS('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			
			a_class = 'AnalyzeGroup_ST_BUD';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the graph analysis with structural data at fixed density.
			%
			% LIST = AnalyzeGroup_ST_BUD.GETSUBCLASSES() returns all subclasses of 'AnalyzeGroup_ST_BUD'.
			%
			% Alternative forms to call this method are:
			%  LIST = A.GETSUBCLASSES() returns all subclasses of the graph analysis with structural data at fixed density A.
			%  LIST = Element.GETSUBCLASSES(A) returns all subclasses of 'A'.
			%  LIST = Element.GETSUBCLASSES('AnalyzeGroup_ST_BUD') returns all subclasses of 'AnalyzeGroup_ST_BUD'.
			%
			% Note that the Element.GETSUBCLASSES(A) and Element.GETSUBCLASSES('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'AnalyzeGroup_ST_BUD' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of graph analysis with structural data at fixed density.
			%
			% PROPS = AnalyzeGroup_ST_BUD.GETPROPS() returns the property list of graph analysis with structural data at fixed density
			%  as a row vector.
			%
			...
			%
			% Alternative forms to call this method are:
			%  PROPS = A.GETPROPS([CATEGORY]) returns the property list of the graph analysis with structural data at fixed density A.
			...
			%
			% Note that the Element.GETPROPS(A) and Element.GETPROPS('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			%GETPROPNUMBER returns the property number of graph analysis with structural data at fixed density.
			%
			% N = AnalyzeGroup_ST_BUD.GETPROPNUMBER() returns the property number of graph analysis with structural data at fixed density.
			%
			% N = AnalyzeGroup_ST_BUD.GETPROPNUMBER(CATEGORY) returns the property number of graph analysis with structural data at fixed density
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER([CATEGORY]) returns the property number of the graph analysis with structural data at fixed density A.
			...
			%
			% Note that the Element.GETPROPNUMBER(A) and Element.GETPROPNUMBER('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			%EXISTSPROP checks whether property exists in graph analysis with structural data at fixed density/error.
			%
			...
			%   Error id: [BRAPH2:AnalyzeGroup_ST_BUD:WrongInput]
			%
			% Note that the Element.EXISTSPROP(A) and Element.EXISTSPROP('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_ST_BUD:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_ST_BUD:' 'WrongInput' '\n' ...
		...
		function check_out = existsTag(tag)
			%EXISTSTAG checks whether tag exists in graph analysis with structural data at fixed density/error.
			%
			...
			%   Error id: [BRAPH2:AnalyzeGroup_ST_BUD:WrongInput]
			%
			% Note that the Element.EXISTSTAG(A) and Element.EXISTSTAG('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%
			% Note that the Element.GETPROPPROP(A) and Element.GETPROPPROP('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%
			% Note that the Element.GETPROPTAG(A) and Element.GETPROPTAG('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%
			% Note that the Element.GETPROPCATEGORY(A) and Element.GETPROPCATEGORY('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_format = getPropFormat(pointer)
			...
			%
			% Note that the Element.GETPROPFORMAT(A) and Element.GETPROPFORMAT('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_description = getPropDescription(pointer)
			...
			%
			% Note that the Element.GETPROPDESCRIPTION(A) and Element.GETPROPDESCRIPTION('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%
			% Note that the Element.GETPROPSETTINGS(A) and Element.GETPROPSETTINGS('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefault(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULT(A) and Element.GETPROPDEFAULT('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
		...
		function prop_default = getPropDefaultConditioned(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(A) and Element.GETPROPDEFAULTCONDITIONED('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%   Error id: BRAPH2:AnalyzeGroup_ST_BUD:WrongInput]
			% 
			% Note that the Element.CHECKPROP(A) and Element.CHECKPROP('AnalyzeGroup_ST_BUD')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_ST_BUD:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_ST_BUD:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeGroup_ST_BUD.getPropTag(prop) ' (' AnalyzeGroup_ST_BUD.getFormatTag(AnalyzeGroup_ST_BUD.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
