classdef AnalyzeGroup_ST_MP_BUT < AnalyzeGroup
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the graph analysis with structural multiplex data of fixed threshold.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the graph analysis with structural multiplex data of fixed threshold.
	...
	%  tostring - string with information about the graph analysis with structural multiplex data of fixed threshold
	%  disp - displays information about the graph analysis with structural multiplex data of fixed threshold
	%  tree - displays the tree of the graph analysis with structural multiplex data of fixed threshold
	%
	% AnalyzeGroup_ST_MP_BUT methods (miscellanea):
	%  getNoValue - returns a pointer to a persistent instance of NoValue
	%               Use it as Element.getNoValue()
	%  getCallback - returns the callback to a property
	%  isequal - determines whether two graph analysis with structural multiplex data of fixed threshold are equal (values, locked)
	%  getElementList - returns a list with all subelements
	%  copy - copies the graph analysis with structural multiplex data of fixed threshold
	%
	% AnalyzeGroup_ST_MP_BUT methods (save/load, Static):
	%  save - saves BRAPH2 graph analysis with structural multiplex data of fixed threshold as b2 file
	%  load - loads a BRAPH2 graph analysis with structural multiplex data of fixed threshold from a b2 file
	%
	% AnalyzeGroup_ST_MP_BUT method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the graph analysis with structural multiplex data of fixed threshold
	%
	% AnalyzeGroup_ST_MP_BUT method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the graph analysis with structural multiplex data of fixed threshold
	%
	...
	%  getSubclasses - returns all subclasses of AnalyzeGroup_ST_MP_BUT
	%  getProps - returns the property list of the graph analysis with structural multiplex data of fixed threshold
	%  getPropNumber - returns the property number of the graph analysis with structural multiplex data of fixed threshold
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?AnalyzeGroup_ST_MP_BUT; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">AnalyzeGroup_ST_MP_BUT constants</a>.
	%
	%
	...
	methods % constructor
		function a = AnalyzeGroup_ST_MP_BUT(varargin)
			...
			%
			% AnalyzeGroup_ST_MP_BUT(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the graph analysis with structural multiplex data of fixed threshold.
			%
			% BUILD = AnalyzeGroup_ST_MP_BUT.GETBUILD() returns the build of 'AnalyzeGroup_ST_MP_BUT'.
			%
			% Alternative forms to call this method are:
			%  BUILD = A.GETBUILD() returns the build of the graph analysis with structural multiplex data of fixed threshold A.
			%  BUILD = Element.GETBUILD(A) returns the build of 'A'.
			%  BUILD = Element.GETBUILD('AnalyzeGroup_ST_MP_BUT') returns the build of 'AnalyzeGroup_ST_MP_BUT'.
			%
			% Note that the Element.GETBUILD(A) and Element.GETBUILD('AnalyzeGroup_ST_MP_BUT')
		...
		function a_class = getClass()
			...
			%
			% Note that the Element.GETCLASS(A) and Element.GETCLASS('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			
			a_class = 'AnalyzeGroup_ST_MP_BUT';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the graph analysis with structural multiplex data of fixed threshold.
			%
			% LIST = AnalyzeGroup_ST_MP_BUT.GETSUBCLASSES() returns all subclasses of 'AnalyzeGroup_ST_MP_BUT'.
			%
			% Alternative forms to call this method are:
			%  LIST = A.GETSUBCLASSES() returns all subclasses of the graph analysis with structural multiplex data of fixed threshold A.
			%  LIST = Element.GETSUBCLASSES(A) returns all subclasses of 'A'.
			%  LIST = Element.GETSUBCLASSES('AnalyzeGroup_ST_MP_BUT') returns all subclasses of 'AnalyzeGroup_ST_MP_BUT'.
			%
			% Note that the Element.GETSUBCLASSES(A) and Element.GETSUBCLASSES('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'AnalyzeGroup_ST_MP_BUT' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			...
			%
			% Note that the Element.GETPROPS(A) and Element.GETPROPS('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = AnalyzeGroup_ST_MP_BUT.GETPROPNUMBER(CATEGORY) returns the property number of graph analysis with structural multiplex data of fixed threshold
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = A.GETPROPNUMBER([CATEGORY]) returns the property number of the graph analysis with structural multiplex data of fixed threshold A.
			...
			%
			% Note that the Element.GETPROPNUMBER(A) and Element.GETPROPNUMBER('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_ST_MP_BUT:WrongInput]
			%
			% Note that the Element.EXISTSPROP(A) and Element.EXISTSPROP('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_ST_MP_BUT:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_ST_MP_BUT:' 'WrongInput' '\n' ...
		...
		function check_out = existsTag(tag)
			...
			%   Error id: [BRAPH2:AnalyzeGroup_ST_MP_BUT:WrongInput]
			%
			% Note that the Element.EXISTSTAG(A) and Element.EXISTSTAG('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%
			% Note that the Element.GETPROPPROP(A) and Element.GETPROPPROP('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%
			% Note that the Element.GETPROPTAG(A) and Element.GETPROPTAG('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				analyzegroup_st_mp_but_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'GR'  'G'  'CORRELATION_RULE'  'NEGATIVE_WEIGHT_RULE'  'THRESHOLDS' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%
			% Note that the Element.GETPROPCATEGORY(A) and Element.GETPROPCATEGORY('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzegroup_st_mp_but_category_list = { 1  1  1  3  4  2  2  6  9  4  5  3  3  3 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%
			% Note that the Element.GETPROPFORMAT(A) and Element.GETPROPFORMAT('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzegroup_st_mp_but_format_list = { 2  2  2  8  2  2  2  2  4  8  8  5  5  12 };
		...
		function prop_description = getPropDescription(pointer)
			...
			%
			% Note that the Element.GETPROPDESCRIPTION(A) and Element.GETPROPDESCRIPTION('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%
			% Note that the Element.GETPROPSETTINGS(A) and Element.GETPROPSETTINGS('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 12 % AnalyzeGroup_ST_MP_BUT.CORRELATION_RULE
					prop_settings = Correlation.CORRELATION_RULE_LIST;
				case 13 % AnalyzeGroup_ST_MP_BUT.NEGATIVE_WEIGHT_RULE
					prop_settings = Correlation.NEGATIVE_WEIGHT_RULE_LIST;
				case 14 % AnalyzeGroup_ST_MP_BUT.THRESHOLDS
					prop_settings = Format.getFormatSettings(12);
				case 4 % AnalyzeGroup_ST_MP_BUT.TEMPLATE
					prop_settings = 'AnalyzeGroup_ST_MP_BUT';
				case 11 % AnalyzeGroup_ST_MP_BUT.G
		...
		function prop_default = getPropDefault(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULT(A) and Element.GETPROPDEFAULT('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 14 % AnalyzeGroup_ST_MP_BUT.THRESHOLDS
					prop_default = [-1:.5:1];
				case 1 % AnalyzeGroup_ST_MP_BUT.ELCLASS
					prop_default = 'AnalyzeGroup_ST_MP_BUT';
				case 2 % AnalyzeGroup_ST_MP_BUT.NAME
					prop_default = 'Multiplex Structural Binary Undirected at fixed Thresholds Analyze Group';
				case 3 % AnalyzeGroup_ST_MP_BUT.DESCRIPTION
					prop_default = 'This graph analysis uses structural multiplex data of fixed threshold and analyzes them using binary undirected graphs.';
				case 4 % AnalyzeGroup_ST_MP_BUT.TEMPLATE
					prop_default = Format.getFormatDefault(8, AnalyzeGroup_ST_MP_BUT.getPropSettings(prop));
				case 5 % AnalyzeGroup_ST_MP_BUT.ID
					prop_default = 'AnalyzeGroup_ST_MP_BUT ID';
				case 6 % AnalyzeGroup_ST_MP_BUT.LABEL
					prop_default = 'AnalyzeGroup_ST_MP_BUT label';
				case 7 % AnalyzeGroup_ST_MP_BUT.NOTES
					prop_default = 'AnalyzeGroup_ST_MP_BUT notes';
				case 10 % AnalyzeGroup_ST_MP_BUT.GR
		...
		function prop_default = getPropDefaultConditioned(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(A) and Element.GETPROPDEFAULTCONDITIONED('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%   Error id: BRAPH2:AnalyzeGroup_ST_MP_BUT:WrongInput]
			% 
			% Note that the Element.CHECKPROP(A) and Element.CHECKPROP('AnalyzeGroup_ST_MP_BUT')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeGroup_ST_MP_BUT:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeGroup_ST_MP_BUT:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeGroup_ST_MP_BUT.getPropTag(prop) ' (' AnalyzeGroup_ST_MP_BUT.getFormatTag(AnalyzeGroup_ST_MP_BUT.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
