classdef AnalyzeEnsemble_CON_BUT < AnalyzeEnsemble
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the ensemble-based graph analysis using connectivity data of fixed threshold.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the ensemble-based graph analysis using connectivity data of fixed threshold.
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the graph analysis with connectivity data of fixed threshold.
			%
			% BUILD = AnalyzeEnsemble_CON_BUT.GETBUILD() returns the build of 'AnalyzeEnsemble_CON_BUT'.
			%
			% Alternative forms to call this method are:
			%  BUILD = A.GETBUILD() returns the build of the graph analysis with connectivity data of fixed threshold A.
			%  BUILD = Element.GETBUILD(A) returns the build of 'A'.
			%  BUILD = Element.GETBUILD('AnalyzeEnsemble_CON_BUT') returns the build of 'AnalyzeEnsemble_CON_BUT'.
			%
			% Note that the Element.GETBUILD(A) and Element.GETBUILD('AnalyzeEnsemble_CON_BUT')
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				analyzeensemble_con_but_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'GR'  'GRAPH_TEMPLATE'  'G_DICT'  'ME_DICT'  'MEASUREENSEMBLE'  'THRESHOLDS' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			analyzeensemble_con_but_category_list = { 1  1  1  3  4  2  2  6  9  4  3  5  5  6  3 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			analyzeensemble_con_but_format_list = { 2  2  2  8  2  2  2  2  4  8  8  10  10  8  12 };
		...
		function prop_settings = getPropSettings(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 15 % AnalyzeEnsemble_CON_BUT.THRESHOLDS
					prop_settings = Format.getFormatSettings(12);
				case 4 % AnalyzeEnsemble_CON_BUT.TEMPLATE
					prop_settings = 'AnalyzeEnsemble_CON_BUT';
				case 11 % AnalyzeEnsemble_CON_BUT.GRAPH_TEMPLATE
					prop_settings = 'MultigraphBUT';
				case 12 % AnalyzeEnsemble_CON_BUT.G_DICT
					prop_settings = 'MultigraphBUT';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				case 15 % AnalyzeEnsemble_CON_BUT.THRESHOLDS
					prop_default = [-1:.5:1];
				case 1 % AnalyzeEnsemble_CON_BUT.ELCLASS
					prop_default = 'AnalyzeEnsemble_CON_BUT';
				case 2 % AnalyzeEnsemble_CON_BUT.NAME
					prop_default = 'Connectivity Binary Undirected at fixed Thresholds Analyze Ensemble';
				case 3 % AnalyzeEnsemble_CON_BUT.DESCRIPTION
				...
				case 5 % AnalyzeEnsemble_CON_BUT.ID
					prop_default = 'AnalyzeEnsemble_CON_BUT ID';
				case 6 % AnalyzeEnsemble_CON_BUT.LABEL
					prop_default = 'AnalyzeEnsemble_CON_BUT label';
				case 7 % AnalyzeEnsemble_CON_BUT.NOTES
					prop_default = 'AnalyzeEnsemble_CON_BUT notes';
				case 10 % AnalyzeEnsemble_CON_BUT.GR
