classdef AnalyzeEnsemble_CON_WU < AnalyzeEnsemble
	%AnalyzeEnsemble_CON_WU is a ensemble-based graph analysis using connectivity data.
	...
	%  <strong>1</strong> <strong>ELCLASS</strong> 	ELCLASS (constant, string) is the class of the ensemble-based graph analysis using connectivity data.
	%  <strong>2</strong> <strong>NAME</strong> 	NAME (constant, string) is the name of the ensemble-based graph analysis using connectivity data.
	%  <strong>3</strong> <strong>DESCRIPTION</strong> 	DESCRIPTION (constant, string) is the description of the ensemble-based graph analysis using connectivity data.
	%  <strong>4</strong> <strong>TEMPLATE</strong> 	TEMPLATE (parameter, item) is the template of the ensemble-based graph analysis using connectivity data.
	%  <strong>5</strong> <strong>ID</strong> 	ID (data, string) is a few-letter code for the ensemble-based graph analysis using connectivity data.
	%  <strong>6</strong> <strong>LABEL</strong> 	LABEL (metadata, string) is an extended label of the ensemble-based graph analysis using connectivity data.
	%  <strong>7</strong> <strong>NOTES</strong> 	NOTES (metadata, string) are some specific notes about the ensemble-based graph analysis using connectivity data.
	...
	%
	% AnalyzeEnsemble_CON_WU methods (miscellanea):
	...
	%
	% AnalyzeEnsemble_CON_WU methods (save/load, Static):
	...
	%  getSubclasses - returns all subclasses of AnalyzeEnsemble_CON_WU
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?AnalyzeEnsemble_CON_WU; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">AnalyzeEnsemble_CON_WU constants</a>.
	%
	%
	...
	methods % constructor
		function a = AnalyzeEnsemble_CON_WU(varargin)
			...
			%
			% AnalyzeEnsemble_CON_WU(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		...
		function a_class = getClass()
			...
			%
			% Note that the Element.GETCLASS(A) and Element.GETCLASS('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
		...
		function subclass_list = getSubclasses()
			...
			%
			% LIST = AnalyzeEnsemble_CON_WU.GETSUBCLASSES() returns all subclasses of 'AnalyzeEnsemble_CON_WU'.
			%
			% Alternative forms to call this method are:
			...
			%  LIST = Element.GETSUBCLASSES('AnalyzeEnsemble_CON_WU') returns all subclasses of 'AnalyzeEnsemble_CON_WU'.
			%
			% Note that the Element.GETSUBCLASSES(A) and Element.GETSUBCLASSES('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'AnalyzeEnsemble_CON_WU' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			...
			%
			% Note that the Element.GETPROPS(A) and Element.GETPROPS('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = AnalyzeEnsemble_CON_WU.GETPROPNUMBER(CATEGORY) returns the property number of graph analysis with connectivity data
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			...
			%
			% Note that the Element.GETPROPNUMBER(A) and Element.GETPROPNUMBER('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%   Error id: [BRAPH2:AnalyzeEnsemble_CON_WU:WrongInput]
			%
			% Note that the Element.EXISTSPROP(A) and Element.EXISTSPROP('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeEnsemble_CON_WU:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeEnsemble_CON_WU:' 'WrongInput' '\n' ...
		...
		function check_out = existsTag(tag)
			...
			%   Error id: [BRAPH2:AnalyzeEnsemble_CON_WU:WrongInput]
			%
			% Note that the Element.EXISTSTAG(A) and Element.EXISTSTAG('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
		...
		function prop = getPropProp(pointer)
			...
			%
			% Note that the Element.GETPROPPROP(A) and Element.GETPROPPROP('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%
			% Note that the Element.GETPROPTAG(A) and Element.GETPROPTAG('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				analyzeensemble_con_wu_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'WAITBAR'  'GR'  'GRAPH_TEMPLATE'  'G_DICT'  'ME_DICT'  'MEASUREENSEMBLE' };
				tag = analyzeensemble_con_wu_tag_list{pointer}; % prop = pointer
			end
		end
		function prop_category = getPropCategory(pointer)
			...
			%
			% Note that the Element.GETPROPCATEGORY(A) and Element.GETPROPCATEGORY('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzeensemble_con_wu_category_list = { 1  1  1  3  4  2  2  6  9  4  3  5  5  6 };
			prop_category = analyzeensemble_con_wu_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%
			% Note that the Element.GETPROPFORMAT(A) and Element.GETPROPFORMAT('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			analyzeensemble_con_wu_format_list = { 2  2  2  8  2  2  2  2  4  8  8  10  10  8 };
			prop_format = analyzeensemble_con_wu_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%
			% Note that the Element.GETPROPDESCRIPTION(A) and Element.GETPROPDESCRIPTION('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%
			% Note that the Element.GETPROPSETTINGS(A) and Element.GETPROPSETTINGS('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % AnalyzeEnsemble_CON_WU.TEMPLATE
					prop_settings = 'AnalyzeEnsemble_CON_WU';
				case 11 % AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE
					prop_settings = 'GraphWU';
				case 12 % AnalyzeEnsemble_CON_WU.G_DICT
					prop_settings = 'GraphWU';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULT(A) and Element.GETPROPDEFAULT('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % AnalyzeEnsemble_CON_WU.ELCLASS
					prop_default = 'AnalyzeEnsemble_CON_WU';
				case 2 % AnalyzeEnsemble_CON_WU.NAME
					prop_default = 'Connectivity Weigthed Undirected Analyze Ensemble';
				case 3 % AnalyzeEnsemble_CON_WU.DESCRIPTION
					prop_default = 'This ensemble-based graph analysis (AnalyzeEnsemble_CON_WU) analyzes connectivity data using weighted undirected graphs.';
				case 4 % AnalyzeEnsemble_CON_WU.TEMPLATE
					prop_default = Format.getFormatDefault(8, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case 5 % AnalyzeEnsemble_CON_WU.ID
					prop_default = 'AnalyzeEnsemble_CON_WU ID';
				case 6 % AnalyzeEnsemble_CON_WU.LABEL
					prop_default = 'AnalyzeEnsemble_CON_WU label';
				case 7 % AnalyzeEnsemble_CON_WU.NOTES
					prop_default = 'AnalyzeEnsemble_CON_WU notes';
				case 10 % AnalyzeEnsemble_CON_WU.GR
					prop_default = Group('SUB_CLASS', 'SubjectCON');
				case 11 % AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE
					prop_default = Format.getFormatDefault(8, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case 12 % AnalyzeEnsemble_CON_WU.G_DICT
					prop_default = Format.getFormatDefault(10, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			...
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(A) and Element.GETPROPDEFAULTCONDITIONED('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
	...
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%   Error id: BRAPH2:AnalyzeEnsemble_CON_WU:WrongInput]
			% 
			% Note that the Element.CHECKPROP(A) and Element.CHECKPROP('AnalyzeEnsemble_CON_WU')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 4 % AnalyzeEnsemble_CON_WU.TEMPLATE
					check = Format.checkFormat(8, value, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case 11 % AnalyzeEnsemble_CON_WU.GRAPH_TEMPLATE
					check = Format.checkFormat(8, value, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				case 12 % AnalyzeEnsemble_CON_WU.G_DICT
					check = Format.checkFormat(10, value, AnalyzeEnsemble_CON_WU.getPropSettings(prop));
				otherwise
					if prop <= 14
			...
			elseif ~check
				error( ...
					['BRAPH2' ':AnalyzeEnsemble_CON_WU:' 'WrongInput'], ...
					['BRAPH2' ':AnalyzeEnsemble_CON_WU:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' AnalyzeEnsemble_CON_WU.getPropTag(prop) ' (' AnalyzeEnsemble_CON_WU.getFormatTag(AnalyzeEnsemble_CON_WU.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
