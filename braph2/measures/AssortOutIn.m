classdef AssortOutIn < Measure
	...
	methods (Static) % inspection
		function build = getBuild()
			%GETBUILD returns the build of the out-in-assortativity.
			%
			% BUILD = AssortOutIn.GETBUILD() returns the build of 'AssortOutIn'.
			%
			% Alternative forms to call this method are:
			%  BUILD = M.GETBUILD() returns the build of the out-in-assortativity M.
			%  BUILD = Element.GETBUILD(M) returns the build of 'M'.
			%  BUILD = Element.GETBUILD('AssortOutIn') returns the build of 'AssortOutIn'.
			%
			% Note that the Element.GETBUILD(M) and Element.GETBUILD('AssortOutIn')
		...
		function tag = getPropTag(pointer)
			...
			if ischar(pointer)
				...
				%CET: Computational Efficiency Trick
				assortoutin_tag_list = { 'ELCLASS'  'NAME'  'DESCRIPTION'  'TEMPLATE'  'ID'  'LABEL'  'NOTES'  'TOSTRING'  'SHAPE'  'SCOPE'  'PARAMETRICITY'  'COMPATIBLE_GRAPHS'  'G'  'M'  'PFM' };
		...
		function prop_category = getPropCategory(pointer)
			...
			%CET: Computational Efficiency Trick
			assortoutin_category_list = { 1  1  1  3  4  2  2  6  1  1  1  1  4  5  9 };
		...
		function prop_format = getPropFormat(pointer)
			...
			%CET: Computational Efficiency Trick
			assortoutin_format_list = { 2  2  2  8  2  2  2  2  11  11  11  7  8  16  8 };
		...
		function prop_default = getPropDefault(pointer)
			...
			switch prop %CET: Computational Efficiency Trick
				...
				case 10 % AssortOutIn.SCOPE
					prop_default = 2;
				case 11 % AssortOutIn.PARAMETRICITY
					prop_default = 2;
				case 12 % AssortOutIn.COMPATIBLE_GRAPHS
