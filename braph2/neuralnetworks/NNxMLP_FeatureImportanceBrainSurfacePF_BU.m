classdef NNxMLP_FeatureImportanceBrainSurfacePF_BU < NNxMLP_FeatureImportanceBrainSurfacePF
	%NNxMLP_FeatureImportanceBrainSurfacePF_BU is a plot of a feature importance brain.
	...
	%
	% The list of NNxMLP_FeatureImportanceBrainSurfacePF_BU properties is:
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (constructor):
	%  NNxMLP_FeatureImportanceBrainSurfacePF_BU - constructor
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods:
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (display):
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (miscellanea):
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (save/load, Static):
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU method (JSON encode):
	%  encodeJSON - returns a JSON string encoding the panel figure for feature importance brain
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU method (JSON decode, Static):
	%   decodeJSON - returns a JSON string encoding the panel figure for feature importance brain
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (inspection, Static):
	%  getClass - returns the class of the panel figure for feature importance brain
	%  getSubclasses - returns all subclasses of NNxMLP_FeatureImportanceBrainSurfacePF_BU
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (GUI):
	%  getPanelProp - returns a prop panel
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (GUI, Static):
	%  getGUIMenuImport - returns the importer menu
	%  getGUIMenuExport - returns the exporter menu
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (category, Static):
	...
	%
	% NNxMLP_FeatureImportanceBrainSurfacePF_BU methods (format, Static):
	...
	%
	% To print full list of constants, click here <a href="matlab:metaclass = ?NNxMLP_FeatureImportanceBrainSurfacePF_BU; properties = metaclass.PropertyList;for i = 1:1:length(properties), if properties(i).Constant, disp([properties(i).Name newline() tostring(properties(i).DefaultValue) newline()]), end, end">NNxMLP_FeatureImportanceBrainSurfacePF_BU constants</a>.
	%
	%
	...
	methods % constructor
		function pf = NNxMLP_FeatureImportanceBrainSurfacePF_BU(varargin)
			%NNxMLP_FeatureImportanceBrainSurfacePF_BU() creates a panel figure for feature importance brain.
			%
			% NNxMLP_FeatureImportanceBrainSurfacePF_BU(PROP, VALUE, ...) with property PROP initialized to VALUE.
			%
			% NNxMLP_FeatureImportanceBrainSurfacePF_BU(TAG, VALUE, ...) with property TAG set to VALUE.
			%
	...
	methods (Static) % inspection
		function build = getBuild()
			...
			%
			% BUILD = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETBUILD() returns the build of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Alternative forms to call this method are:
			%  BUILD = PF.GETBUILD() returns the build of the panel figure for feature importance brain PF.
			%  BUILD = Element.GETBUILD(PF) returns the build of 'PF'.
			%  BUILD = Element.GETBUILD('NNxMLP_FeatureImportanceBrainSurfacePF_BU') returns the build of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Note that the Element.GETBUILD(PF) and Element.GETBUILD('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
		...
		function pf_class = getClass()
			%GETCLASS returns the class of the panel figure for feature importance brain.
			%
			% CLASS = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETCLASS() returns the class 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Alternative forms to call this method are:
			%  CLASS = PF.GETCLASS() returns the class of the panel figure for feature importance brain PF.
			%  CLASS = Element.GETCLASS(PF) returns the class of 'PF'.
			%  CLASS = Element.GETCLASS('NNxMLP_FeatureImportanceBrainSurfacePF_BU') returns 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Note that the Element.GETCLASS(PF) and Element.GETCLASS('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			
			pf_class = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU';
		end
		function subclass_list = getSubclasses()
			%GETSUBCLASSES returns all subclasses of the panel figure for feature importance brain.
			%
			% LIST = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETSUBCLASSES() returns all subclasses of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Alternative forms to call this method are:
			%  LIST = PF.GETSUBCLASSES() returns all subclasses of the panel figure for feature importance brain PF.
			%  LIST = Element.GETSUBCLASSES(PF) returns all subclasses of 'PF'.
			%  LIST = Element.GETSUBCLASSES('NNxMLP_FeatureImportanceBrainSurfacePF_BU') returns all subclasses of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Note that the Element.GETSUBCLASSES(PF) and Element.GETSUBCLASSES('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			% See also subclasses.
			
			subclass_list = { 'NNxMLP_FeatureImportanceBrainSurfacePF_BU' }; %CET: Computational Efficiency Trick
		end
		function prop_list = getProps(category)
			%GETPROPS returns the property list of panel figure for feature importance brain.
			%
			% PROPS = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPS() returns the property list of panel figure for feature importance brain
			%  as a row vector.
			%
			% PROPS = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPS(CATEGORY) returns the property list 
			...
			%  PROPS = Element.GETPROPS('NNxMLP_FeatureImportanceBrainSurfacePF_BU'[, CATEGORY]) returns the property list of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Note that the Element.GETPROPS(PF) and Element.GETPROPS('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_number = getPropNumber(varargin)
			...
			%
			% N = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPNUMBER() returns the property number of panel figure for feature importance brain.
			%
			% N = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPNUMBER(CATEGORY) returns the property number of panel figure for feature importance brain
			%  of category CATEGORY
			%
			% Alternative forms to call this method are:
			%  N = PF.GETPROPNUMBER([CATEGORY]) returns the property number of the panel figure for feature importance brain PF.
			%  N = Element.GETPROPNUMBER(PF) returns the property number of 'PF'.
			%  N = Element.GETPROPNUMBER('NNxMLP_FeatureImportanceBrainSurfacePF_BU') returns the property number of 'NNxMLP_FeatureImportanceBrainSurfacePF_BU'.
			%
			% Note that the Element.GETPROPNUMBER(PF) and Element.GETPROPNUMBER('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
		...
		function check_out = existsProp(prop)
			...
			%
			% CHECK = NNxMLP_FeatureImportanceBrainSurfacePF_BU.EXISTSPROP(PROP) checks whether the property PROP exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSPROP(PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(PF, PROP) checks whether PROP exists for PF.
			%  CHECK = Element.EXISTSPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP) checks whether PROP exists for NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Element.EXISTSPROP(PROP) throws an error if the PROP does NOT exist.
			%  Error id: [BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			%
			% Alternative forms to call this method are:
			...
			%   Error id: [BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			%  Element.EXISTSPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP) throws error if PROP does NOT exist for NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%   Error id: [BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			%
			% Note that the Element.EXISTSPROP(PF) and Element.EXISTSPROP('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			elseif ~check
				error( ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput'], ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput' '\n' ...
					'The value ' tostring(prop, 100, ' ...') ' is not a valid prop for NNxMLP_FeatureImportanceBrainSurfacePF_BU.'] ...
					)
			end
		end
		function check_out = existsTag(tag)
			...
			%
			% CHECK = NNxMLP_FeatureImportanceBrainSurfacePF_BU.EXISTSTAG(TAG) checks whether a property with tag TAG exists.
			%
			% Alternative forms to call this method are:
			%  CHECK = PF.EXISTSTAG(TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(PF, TAG) checks whether TAG exists for PF.
			%  CHECK = Element.EXISTSTAG(NNxMLP_FeatureImportanceBrainSurfacePF_BU, TAG) checks whether TAG exists for NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			...
			%   Error id: [BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			%  Element.EXISTSTAG(NNxMLP_FeatureImportanceBrainSurfacePF_BU, TAG) throws error if TAG does NOT exist for NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%   Error id: [BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			%
			% Note that the Element.EXISTSTAG(PF) and Element.EXISTSTAG('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			% See also getProps, existsTag.
			...
			elseif ~check
				error( ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput'], ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput' '\n' ...
					'The value ' tag ' is not a valid tag for NNxMLP_FeatureImportanceBrainSurfacePF_BU.'] ...
					)
			end
		end
		function prop = getPropProp(pointer)
			...
			%  PROPERTY = Element.GETPROPPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns property number of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  PROPERTY = PF.GETPROPPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns property number of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPPROP(PF) and Element.GETPROPPROP('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
		...
		function tag = getPropTag(pointer)
			...
			%  TAG = Element.GETPROPTAG(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns tag of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  TAG = PF.GETPROPTAG(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns tag of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPTAG(PF) and Element.GETPROPTAG('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_category = getPropCategory(pointer)
			...
			%  CATEGORY = Element.GETPROPCATEGORY(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns category of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  CATEGORY = PF.GETPROPCATEGORY(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns category of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPCATEGORY(PF) and Element.GETPROPCATEGORY('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			% See also Category, getPropProp, getPropTag, getPropSettings,
			%  getPropFormat, getPropDescription, getPropDefault, checkProp.
			
			prop = NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropProp(pointer);
			
			%CET: Computational Efficiency Trick
			nnxmlp_featureimportancebrainsurfacepf_bu_category_list = { 1  1  1  3  4  2  2  6  9  7  6  6  9  8  7  6  6  6  6  8  7  7  7  8  8  7  8  2  7  8  8  8  2  7  8  8  7  8  8  7  8  8  7  8  8  2  2  6 };
			prop_category = nnxmlp_featureimportancebrainsurfacepf_bu_category_list{prop};
		end
		function prop_format = getPropFormat(pointer)
			...
			%  FORMAT = Element.GETPROPFORMAT(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns format of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  FORMAT = PF.GETPROPFORMAT(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns format of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPFORMAT(PF) and Element.GETPROPFORMAT('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			%CET: Computational Efficiency Trick
			nnxmlp_featureimportancebrainsurfacepf_bu_format_list = { 2  2  2  8  2  2  2  2  4  18  4  4  8  20  18  4  4  4  4  8  18  19  18  12  8  18  5  8  18  4  8  8  8  19  4  10  19  4  10  19  4  10  19  4  10  16  8  1 };
			prop_format = nnxmlp_featureimportancebrainsurfacepf_bu_format_list{prop};
		end
		function prop_description = getPropDescription(pointer)
			...
			%  DESCRIPTION = Element.GETPROPDESCRIPTION(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns description of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  DESCRIPTION = PF.GETPROPDESCRIPTION(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns description of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPDESCRIPTION(PF) and Element.GETPROPDESCRIPTION('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
		...
		function prop_settings = getPropSettings(pointer)
			...
			%  SETTINGS = Element.GETPROPSETTINGS(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns settings of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  SETTINGS = PF.GETPROPSETTINGS(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns settings of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPSETTINGS(PF) and Element.GETPROPSETTINGS('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 4 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.TEMPLATE
					prop_settings = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU';
				otherwise
		...
		function prop_default = getPropDefault(pointer)
			%GETPROPDEFAULT returns the default value of a property.
			%
			% DEFAULT = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPDEFAULT(PROP) returns the default 
			%  value of the property PROP.
			%
			% DEFAULT = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPDEFAULT(TAG) returns the default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULT(POINTER) returns the default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULT(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns the default value of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  DEFAULT = PF.GETPROPDEFAULT(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns the default value of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPDEFAULT(PF) and Element.GETPROPDEFAULT('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop %CET: Computational Efficiency Trick
				case 1 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.ELCLASS
					prop_default = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU';
				case 2 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.NAME
					prop_default = 'Panel Figure for Measure Group Brain';
				case 3 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.DESCRIPTION
				...
				case 4 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.TEMPLATE
					prop_default = Format.getFormatDefault(8, NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropSettings(prop));
				case 5 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.ID
					prop_default = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU ID';
				case 6 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.LABEL
					prop_default = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU label';
				case 7 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.NOTES
					prop_default = 'NNxMLP_FeatureImportanceBrainSurfacePF_BU notes';
				otherwise
		...
		function prop_default = getPropDefaultConditioned(pointer)
			%GETPROPDEFAULTCONDITIONED returns the conditioned default value of a property.
			%
			% DEFAULT = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPDEFAULTCONDITIONED(PROP) returns the conditioned default 
			%  value of the property PROP.
			%
			% DEFAULT = NNxMLP_FeatureImportanceBrainSurfacePF_BU.GETPROPDEFAULTCONDITIONED(TAG) returns the conditioned default 
			%  value of the property with tag TAG.
			%
			% Alternative forms to call this method are (POINTER = PROP or TAG):
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(POINTER) returns the conditioned default value of POINTER of PF.
			%  DEFAULT = Element.GETPROPDEFAULTCONDITIONED(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns the conditioned default value of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  DEFAULT = PF.GETPROPDEFAULTCONDITIONED(NNxMLP_FeatureImportanceBrainSurfacePF_BU, POINTER) returns the conditioned default value of POINTER of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%
			% Note that the Element.GETPROPDEFAULTCONDITIONED(PF) and Element.GETPROPDEFAULTCONDITIONED('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			prop_default = NNxMLP_FeatureImportanceBrainSurfacePF_BU.conditioning(prop, NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropDefault(prop));
		end
	end
	methods (Static) % checkProp
		function prop_check = checkProp(pointer, value)
			...
			%  CHECK = Element.CHECKPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP, VALUE) checks VALUE format for PROP of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%  CHECK = PF.CHECKPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP, VALUE) checks VALUE format for PROP of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			% 
			...
			%   Error id: BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput
			%  Element.CHECKPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP, VALUE) throws error if VALUE has not a valid format for PROP of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%   Error id: BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput
			%  PF.CHECKPROP(NNxMLP_FeatureImportanceBrainSurfacePF_BU, PROP, VALUE) throws error if VALUE has not a valid format for PROP of NNxMLP_FeatureImportanceBrainSurfacePF_BU.
			%   Error id: BRAPH2:NNxMLP_FeatureImportanceBrainSurfacePF_BU:WrongInput]
			% 
			% Note that the Element.CHECKPROP(PF) and Element.CHECKPROP('NNxMLP_FeatureImportanceBrainSurfacePF_BU')
			%  are less computationally efficient.
			%
			...
			switch prop
				case 4 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.TEMPLATE
					check = Format.checkFormat(8, value, NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropSettings(prop));
				otherwise
					if prop <= 48
			...
			elseif ~check
				error( ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput'], ...
					['BRAPH2' ':NNxMLP_FeatureImportanceBrainSurfacePF_BU:' 'WrongInput' '\n' ...
					'The value ' tostring(value, 100, ' ...') ' is not a valid property ' NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropTag(prop) ' (' NNxMLP_FeatureImportanceBrainSurfacePF_BU.getFormatTag(NNxMLP_FeatureImportanceBrainSurfacePF_BU.getPropFormat(prop)) ').'] ...
					)
			end
		end
	end
	methods (Access=protected) % calculate value
		function value = calculateValue(pf, prop, varargin)
			...
			switch prop
				case 11 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.DRAW
				...
				case 18 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.DELETE
					value = calculateValue@BrainAtlasPF(pf, 18, varargin{:}); % also warning
					if value
					    % do nothing
					end
					
				case 48 % NNxMLP_FeatureImportanceBrainSurfacePF_BU.SETUP
					warning([BRAPH2.STR ':NNxMLP_FeatureImportanceBrainSurfacePF_BU'], [BRAPH2.STR ':NNxMLP_FeatureImportanceBrainSurfacePF_BU \nThis functionality is not implemented yet.\nYou can contact the BRAPH2 developers and ask for it, \nor, even better, implement it yourself and share it with the community!'])
					value = [];
